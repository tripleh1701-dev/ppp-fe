 ...
 * ============================================================================
 * 
 * This file currently uses localStorage to persist credentials data for testing.
 * 
 * TO REVERT TO API-BASED STORAGE:
 * 1. Search for "LOCAL_STORAGE_CREDENTIALS" to find all related code
 ...
 * 6. Update loadCredentials function to call actual API endpoints
 * 7. Update all setCredentials calls to use API save functions instead
 * 
 * The localStorage key used is: 'credentials_credentials_data'
 * ============================================================================
 */
...
import ManageCredentialsTable, { CredentialRow } from '@/components/ManageCredentialsTable';
...
export default function ManageCredentials() {
    ...
    // ============================================================================
    // This section stores credentials data in localStorage for testing purposes.
    // TODO: Remove this entire section and replace with API calls when backend APIs are ready.
    // Search for "LOCAL_STORAGE_CREDENTIALS" to find all related code.
    ...
    const getLocalStorageKey = (accountId: string, enterpriseId: string): string => {
        if (accountId && enterpriseId) {
            return `${LOCAL_STORAGE_CREDENTIALS_KEY}_${accountId}_${enterpriseId}`;
        }
        // Fallback to default key if no account/enterprise
        return LOCAL_STORAGE_CREDENTIALS_KEY;
    };
    
    // Load credentials from localStorage for current account/enterprise
    const loadCredentialsFromLocalStorage = useCallback((accountId?: string, enterpriseId?: string): CredentialRow[] => {
        ...
        try {
            ...
            if (stored) {
                ...
                console.log('📦 [LocalStorage] Loaded credentials from localStorage:', parsed.length, 'rows for account/enterprise:', accountId, enterpriseId);
                return parsed;
            }
        } catch (error) {
            console.error('❌ [LocalStorage] Failed to load credentials from localStorage:', error);
        }
        return [];
    }, []);
    
    // Save credentials to localStorage for current account/enterprise
    const saveCredentialsToLocalStorage = useCallback((roles: CredentialRow[], accountId?: string, enterpriseId?: string) => {
        if (!USE_LOCAL_STORAGE || typeof window === 'undefined') return;
        try {
            if (!accountId || !enterpriseId) {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            (window as any).checkCredentialsLocalStorage = () => {
                try {
                    const stored = window.localStorage.getItem(LOCAL_STORAGE_CREDENTIALS_KEY);
                    if (stored) {
                        const parsed = JSON.parse(stored);
                        console.log('📦 [Debug] Credentials in localStorage:', parsed);
                        console.log('📦 [Debug] Count:', parsed.length, 'rows');
                        return parsed;
                    } else {
                        console.log('📦 [Debug] No credentials data in localStorage');
                        return [];
                    }
                } catch (error) {
            ...
            if (stored) {
                try {
                    const parsed = JSON.parse(stored);
                    console.log('📦 [LocalStorage] Found existing credentials data:', parsed.length, 'rows');
                } catch (error) {
                    console.error('❌ [LocalStorage] Error parsing existing data:', error);
                }
            } else {
                console.log('📦 [LocalStorage] No existing credentials data found');
            }
        }
    }, []);
    // ============================================================================
    ...
    const sortConfigsByDisplayOrder = useCallback((configs: CredentialRow[]) => {
    ...
    const [selectedRowForConnector, setSelectedRowForConnector] = useState<CredentialRow | null>(null);
    ...
    useEffect(() => {
        ...
        const checkOAuthCompletion = () => {
            ...
            const credentialName = localStorage.getItem('githubOAuthCredentialName');
            
            if (shouldReopenModal === 'true' && connectorId && credentialName) {
                // Find the row that matches the credential name
                const matchingRow = credentials.find(row => row.credentialName === credentialName);
                
                if (matchingRow) {
                    console.log('🔄 [OAuth] Reopening modal for credential:', credentialName);
    ...
    useEffect(() => {
        const loadSelectedEnterprise = () => {
            try {
                console.log('🐛 [ManageCredentials Page] Loading localStorage values...');
                ...
                console.log('🐛 [ManageCredentials Page] localStorage values:', {
                ...
                console.log('🐛 [ManageCredentials Page] Setting state values:', {
                    enterprise: savedName || '',
    ...
    // Auto-save credentials to localStorage whenever it changes (but not during loading)
    useEffect(() => {
        ...
        // Only save if we have account/enterprise selected and credentials has data
        if (USE_LOCAL_STORAGE && selectedAccountId && selectedEnterpriseId && credentials.length > 0) {
            // Use a small delay to batch multiple rapid updates
            const saveTimer = setTimeout(() => {
                if (!isLoadingRef.current) { // Double-check loading flag
                    saveCredentialsToLocalStorage(credentials, selectedAccountId, selectedEnterpriseId);
                    console.log('💾 [AutoSave] Saved credentials to localStorage:', credentials.length, 'rows');
                    // Log connectors count for debugging
                    const connectorsCount = credentials.reduce((sum, row) => sum + (row.connectors?.length || 0), 0);
    ...
    }, [credentials, selectedAccountId, selectedEnterpriseId, isInitialized, saveCredentialsToLocalStorage]);
    ...
    const [ActiveGroupLabel, setActiveGroupLabel] = useState<
        'None' | 'Credential Name' | 'Description' | 'Workstream' | 'Product' | 'Service'
    >('None');
    
    type ColumnType = 'credentialName' | 'description' | 'entity' | 'product' | 'service' | 'scope' | 'actions';
    ...
    const loadDropdownOptions = useCallback(async () => {
        try {
            ...
            console.log('⚠️ [Credentials] loadDropdownOptions called but APIs removed - use credential APIs instead');

            // Extract unique credential names from current Credentials (local state only)
            const uniqueCredentialNames = Array.from(new Set(credentials.map(credential => credential.credentialName)
                .filter(Boolean)
            )).map((name, index) => ({
            ...
            const uniqueEntities = Array.from(new Set(credentials.map(credential => credential.entity)
                .filter(Boolean)
            )).map((name, index) => ({
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if (filterVisible && isOutsideFilter) {
                ...
                const isFilterEmpty = !currentForm.credentialName && !currentForm.description && !currentForm.entity && !currentForm.product && !currentForm.service;
    ...
    const allCols: ColumnType[] = ['credentialName', 'description', 'entity', 'product', 'service', 'scope'];
    
    // Columns available for sorting
    const sortableCols: ColumnType[] = ['credentialName', 'description', 'entity', 'product', 'service'];
    ...
    const processedConfigs = React.useMemo(() => {
        ...
        if (appliedSearchTerm.trim()) {
            filtered = filtered.filter((config) => {
                ...
                return (
                    config.credentialName?.toLowerCase().includes(searchLower) ||
        ...
        if (activeFilters.credentialName) {
            filtered = filtered.filter(
                (config) => config.credentialName?.toLowerCase().includes(activeFilters.credentialName.toLowerCase()),
            );
        }
        ...
        if (sortColumn && sortDirection && (sortDirection === 'asc' || sortDirection === 'desc')) {
            filtered.sort((a, b) => {
                ...
                switch (sortColumn) {
                    case 'credentialName':
                        valueA = (a.credentialName || '').toString().toLowerCase();
                        valueB = (b.credentialName || '').toString().toLowerCase();
                        break;
                    case 'description':
    ...
    // Load Credentials from database. Accept optional filters so we can request groups
    // for a specific account/enterprise combination: { accountId, accountName, enterpriseId, enterpriseName }
    // NOTE: API calls removed - credentials should use credential-specific APIs instead
    const loadCredentials = useCallback(async (filters?: {
        ...
        try {
            ...
            if (USE_LOCAL_STORAGE) {
                ...
                console.log('⚠️ [Credentials] loadCredentials called but API removed - use credential APIs instead');
                setCredentials([]);
            }
            // ============================================================================
        } catch (error) {
    ...
    useEffect(() => {
        if (USE_LOCAL_STORAGE && isInitialized && selectedAccountId && selectedEnterpriseId) {
            ...
            const loadedRoles = loadCredentialsFromLocalStorage(selectedAccountId, selectedEnterpriseId);
            if (loadedRoles.length > 0) {
                setCredentials(loadedRoles);
                console.log('📦 [Credentials] Initial load from localStorage:', loadedRoles.length, 'rows for account:', selectedAccountId, 'enterprise:', selectedEnterpriseId);
            }
            // Reset loading flag after a short delay to allow state to update
            setTimeout(() => {
                isLoadingRef.current = false;
    ...
    }, [isInitialized, selectedAccountId, selectedEnterpriseId, loadCredentialsFromLocalStorage]);
    ...
    useEffect(() => {
        ...
        console.log('🔄 [ManageCredentials] Loading Credentials with context:', {
            selectedAccountId,
            selectedAccountName,
            selectedEnterprise,
            enterpriseId,
        ...
        // NOTE: API call removed - credentials should use credential-specific APIs instead
        ...
        if (selectedAccountId && currentEnterpriseId) {
            ...
            // ============================================================================
            if (USE_LOCAL_STORAGE) {
                const loadedRoles = loadCredentialsFromLocalStorage(selectedAccountId, currentEnterpriseId);
                setCredentials(loadedRoles);
                console.log('📦 [Credentials] Loaded from localStorage:', loadedRoles.length, 'rows for account:', selectedAccountId, 'enterprise:', currentEnterpriseId);
            } else {
                console.log('⚠️ [Credentials] loadCredentials call removed - use credential APIs instead');
                // loadCredentials({...}); // REMOVED - use credential-specific APIs
            }
            // ============================================================================
            setIsLoading(false);
            // Reset loading flag after a short delay to allow state to update
        ...
        console.log('⚠️ [ManageCredentials] Missing Account or Enterprise selection, clearing table');
        setCredentials([]);
        setIsLoading(false);
        ...
        if (!selectedAccountId) {
            showBlueNotification('Please select an Account from the top-right dropdown to view Credentials', 5000, false);
        }
        // Enterprise notification removed - enterprise is now auto-selected based on account licenses
    }, [selectedAccountId, selectedAccountName, selectedEnterprise, selectedEnterpriseId, isInitialized, loadCredentials, loadCredentialsFromLocalStorage]);
    ...
    useEffect(() => {
        if (!isLoading && credentials.length > 0 && 
            (credentials.length !== credentialsCountRef.current || !dropdownOptionsLoadedRef.current)) {
    ...
    const validateIncompleteRows = () => {
        ...
        const existingRows = credentials.filter((credential: any) => 
            !String(credential.id).startsWith('tmp-')
        );

        // Check for incomplete temporary rows (exclude completely blank rows)
        const incompleteTemporaryRows = temporaryRows.filter((credential: any) => {
            ...
            const isCompletelyBlank = !hascredentialName && !hasEntity && !hasProduct && !hasService;
            if (isCompletelyBlank) return false;

            // Row is incomplete if it has some data but not all required fields
            return !hascredentialName || !hasEntity || !hasProduct || !hasService;
        });

        // Check for incomplete existing rows
        const incompleteExistingRows = existingRows.filter((credential: any) => {
            const hascredentialName = credential.credentialName?.trim();
            const hasEntity = credential.entity?.trim();
            const hasProduct = credential.product?.trim();
            const hasService = credential.service?.trim();
        ...
        if (incompleteRows.length > 0) {
            ...
            incompleteRows.forEach((credential) => {
                if (!credential.credentialName?.trim()) missingFields.add('Credential Name');
    ...
    const handleUpdateField = useCallback((rowId: string, field: string, value: any) => {
        ...
        if (updatedGroup) {
            const hascredentialName = (updatedGroup as any).credentialName?.trim() && (updatedGroup as any).credentialName.trim().length > 0;
            ...
            const isComplete = hascredentialName && hasEntity && hasProduct && hasService;
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            const credentialToDelete = credentials.find(g => g.id === pendingDeleteRowId);
            ...
            if (!String(pendingDeleteRowId).startsWith('tmp-')) {
                try {
                    ...
                    console.log('⚠️ [Credentials] Delete API removed - use credential APIs instead');
                    console.log('🗑️ Would delete credential with ID:', pendingDeleteRowId);
                ...
                if (storedGroups) {
                    try {
                        ...
                        console.log('✅ Temporary Credential deleted from localStorage');
                    } catch (error) {
            ...
            setCredentials((prev) => {
                const updated = prev.filter((credential) => credential.id !== pendingDeleteRowId);
            ...
            showBlueNotification('Failed to delete Credential. Please try again.', 5000, false);
        } finally {
    ...
    const autoSaveNewUserGroup = async (tempRowId: string, updatedGroup?: any) => {
        ...
        const credential = updatedGroup || credentialsRef.current.find((g) => g.id === tempRowId);
        if (!credential) {
        ...
        const isDuplicate = credentialsRef.current.some((existingCredential) => {
            // Skip the current temporary row being saved
            if (existingCredential.id === tempRowId) return false;
            
            // Check if all key fields match
            return existingCredential.credentialName?.toLowerCase().trim() === credential.credentialName?.toLowerCase().trim() &&
                   existingCredential.entity?.toLowerCase().trim() === credential.entity?.toLowerCase().trim() &&
                   existingCredential.product?.toLowerCase().trim() === credential.product?.toLowerCase().trim() &&
                   existingCredential.service?.toLowerCase().trim() === credential.service?.toLowerCase().trim();
        });

        if (isDuplicate) {
            console.error('❌ Duplicate entry detected - Credential with same Credential Name, Workstream, Product, and Service already exists');
        ...
        // Check if a credential with this name already exists in the database (created via + Add button)
        // NOTE: API calls removed - credentials should use credential-specific APIs instead
        console.log('⚠️ [Credentials] Auto-save API calls removed - use credential APIs instead');
        console.log('🔍 [AutoSave] Would check if credential exists:', credential.credentialName);
        ...
        if (oldDisplayOrder !== undefined) {
            ...
            console.log(`📍 [AutoSave] Preserved display order ${oldDisplayOrder} for new credential ID ${newId}`);
        }

        console.log('🎉 [AutoSave] Credential updated locally (API calls removed)');
    ...
    const debouncedAutoSave = useCallback(async () => {
        ...
        const timer = setTimeout(async () => {
            try {
                ...
                const temporaryRows = credentialsRef.current.filter((credential) => {
                    const isTemp = String(credential.id).startsWith('tmp-');
                    if (!isTemp) return false;
                    
                    // Be more strict about what constitutes a complete Credential row
                    const hascredentialName = credential.credentialName?.trim() && credential.credentialName.trim().length > 0;
                    const hasEntity = credential.entity?.trim() && credential.entity.trim().length > 0;
                    const hasProduct = credential.product?.trim() && credential.product.trim().length > 0;
                    const hasService = credential.service?.trim() && credential.service.trim().length > 0;
                    
                    const isComplete = hascredentialName && hasEntity && hasProduct && hasService;
                    
                    if (isTemp && !isComplete) {
                        console.log(`🚫 Skipping incomplete temporary Credential ${credential.id}:`, {
                ...
                const modifiedRows = credentialsRef.current.filter((credential) => {
                    const isExisting = !String(credential.id).startsWith('tmp-');
                    const isModified = modifiedExistingRecordsRef.current.has(String(credential.id));
                    ...
                    if (isExisting && isModified) {
                        ...
                        console.log(`🔍 Checking modified Credential ${credential.id}: isComplete=${isComplete}`, {
                            hascredentialName: !!hascredentialName,
                            hasEntity: !!hasEntity,
                            hasProduct: !!hasProduct,
                            hasService: !!hasService,
                    ...
                    console.log(`🔍 Checking Credential ${credential.id}: isExisting=${isExisting}, isModified=${isModified}`);
                    return false;
                });
                
            console.log(`📊 Found ${temporaryRows.length} complete temporary Credentials to auto-save`);
            console.log(`📊 Found ${modifiedRows.length} modified existing Credentials to auto-save`);
            ...
            const orphanedRecords = Array.from(modifiedExistingRecordsRef.current).filter(recordId => 
                !credentialsRef.current.find(credential => String(credential.id) === recordId)
            );
            ...
            if (totalRowsToSave > 0) {
                console.log('💾 Auto-saving Credentials after 10 seconds of inactivity...', temporaryRows.map(r => r.id));
                ...
                for (const tempRow of temporaryRows) {
                    ...
                    try {
                        ...
                        console.error(`❌ Failed to auto-save new Credential ${tempRow.id}:`, error);
                ...
                for (const modifiedRow of modifiedRows) {
                    console.log(`💾 Saving modified existing Credential: ${modifiedRow.id}`);
                    try {
                        // Check for duplicate entry (same credentialName + entity + product + service as another record)
                        const isDuplicate = credentialsRef.current.some((existingRole) => {
                            ...
                            return existingRole.credentialName?.toLowerCase().trim() === modifiedRow.credentialName?.toLowerCase().trim() &&
                        ...
                        if (isDuplicate) {
                            console.error(`❌ Duplicate entry detected for autosave update: ${modifiedRow.credentialName}`);
                        ...
                        console.log('⚠️ [Credentials] Update API call removed - use credential APIs instead');
                        console.log(`💾 Would update credential ${modifiedRow.id}`);
                        // await api.put(`/api/user-management/roles/${modifiedRow.id}`, {...}); // REMOVED
                        console.log(`✅ Credential ${modifiedRow.id} updated locally (API calls removed)`);
                        successCount++;
                    } catch (error) {
                        console.error(`❌ Failed to save modified Credential ${modifiedRow.id}:`, error);
                        failureCount++;
                    }
                }
                ...
                if (duplicateDetectedRef.current && successCount === 0 && failureCount === 0) {
                    ...
                    console.log('🔄 Reloading Credentials after successful autosave to update IDs...');
                    ...
                    console.log('✅ Reload complete after autosave - checking state:', {
                        ...
                        hasTempRows: credentialsRef.current.some(g => String(g.id).startsWith('tmp-'))
                    });
                } else if (successCount > 0 && failureCount > 0 && !duplicateDetectedRef.current) {
    ...
    }, [selectedAccountId, selectedAccountName, selectedEnterpriseId, selectedEnterprise, showValidationErrors, loadCredentials]);
    ...
    const getUnsavedChanges = () => {
        ...
        console.log('🔍 [getUnsavedChanges] Check:', {
            ...
            hasTempRows,
            tempRowsIds: credentialsRef.current.filter(g => String(g.id).startsWith('tmp-')).map(g => g.id),
    ...
    const getIncompleteRows = () => {
        const incompleteRows = credentials
            .filter((credential: any) => {
                ...
                const isIncomplete = !hascredentialName || !hasEntity || !hasProduct || !hasService;
    ...
    const handleSaveAll = async () => {
        console.log('💾 Save all clicked - validating and saving Credentials...');
        ...
        const modifiedExistingRowsCount = existingRows.filter((credential: any) => 
            modifiedExistingRecords.has(credential.id)
        ).length;
        ...
        if (incompleteRowsData.length > 0) {
            ...
            incompleteRowsData.forEach((credential) => {
                ...
                if (!credential.credentialName?.trim()) allMissingFields.add('Credential Name');
                if (!credential.entity?.trim()) allMissingFields.add('Workstream');
                if (!credential.product?.trim()) allMissingFields.add('Product');
                if (!credential.service?.trim()) allMissingFields.add('Service');
            });
        ...
        try {
            let savedCount = 0;
            const completeTemporaryRows = temporaryRows.filter((credential: any) => {
                const hascredentialName = credential.credentialName?.trim();
                const hasEntity = credential.entity?.trim();
                const hasProduct = credential.product?.trim();
                const hasService = credential.service?.trim();
            ...
            const completeModifiedRows = existingRows.filter((credential: any) => {
                const hascredentialName = credential.credentialName?.trim();
                const hasEntity = credential.entity?.trim();
                const hasProduct = credential.product?.trim();
                const hasService = credential.service?.trim();
                const isModified = modifiedExistingRecords.has(credential.id);
                return hascredentialName && hasEntity && hasProduct && hasService && isModified;
            });
            ...
            for (const tempGroup of completeTemporaryRows) {
                try {
                    // Check for duplicate entry (same credentialName + entity + product + service)
                    const isDuplicate = credentials.some((existingRole: any) => {
                        // Skip the current temporary row being saved
                        if (existingRole.id === tempGroup.id) return false;
                        
                        // Check if all key fields match
                        return existingRole.credentialName?.toLowerCase().trim() === tempGroup.credentialName?.toLowerCase().trim() &&
                    ...
                    if (isDuplicate) {
                        console.error('❌ Duplicate entry detected for:', tempGroup.credentialName);
                    ...
                    // Check if a credential with this name already exists in the database (created via + Add button)
                    // NOTE: API calls removed - credentials should use credential-specific APIs instead
                    console.log('⚠️ [Credentials] Save API calls removed - use credential APIs instead');
                    console.log('🔍 Would check if credential exists:', tempGroup.credentialName);
                    ...
                    setCredentials((prev) => {
                        ...
                        if (USE_LOCAL_STORAGE && selectedAccountId && selectedEnterpriseId) {
                            ...
                            try {
                                ...
                                console.log('💾 [Save] Row details:', updated.map(r => ({ id: r.id, credentialName: r.credentialName, entity: r.entity, product: r.product, service: r.service })));
                            } catch (error) {
                    ...
                    savedCount++;
                    console.log('🎉 Credential updated locally (API calls removed)');
                } catch (error) {
            ...
            for (const modifiedRole of completeModifiedRows) {
                try {
                    ...
                    const isDuplicate = credentials.some((existingRole: any) => {
                        ...
                        // Check if all key fields match with another existing record
                        return existingRole.credentialName?.toLowerCase().trim() === modifiedRole.credentialName?.toLowerCase().trim() &&
                    ...
                    if (isDuplicate) {
                        console.error('❌ Duplicate entry detected for update:', modifiedRole.credentialName);
                    ...
                    // NOTE: API call removed - credentials should use credential-specific APIs instead
                    console.log('⚠️ [Credentials] Update API call removed - use credential APIs instead');
                    console.log('💾 Would update existing credential:', modifiedRole.id);
            ...
            if (savedCount > 0 && failedCount === 0) {
                ...
                // NOTE: API call removed - credentials should use credential-specific APIs instead
                console.log('⚠️ [Credentials] Reload API call removed - use credential APIs instead');
                // await loadCredentials({...}); // REMOVED
                ...
                if (USE_LOCAL_STORAGE && selectedAccountId && selectedEnterpriseId) {
                    ...
                    setTimeout(() => {
                        ...
                        const currentRoles = credentialsRef.current;
                        if (currentRoles.length > 0) {
                            // Save current state to localStorage
                            saveCredentialsToLocalStorage(currentRoles, selectedAccountId, selectedEnterpriseId);
                            ...
                            console.warn('⚠️ [Save] credentialsRef is empty, checking state...');
                        }
                    }, 200);
                }
                // ============================================================================
    ...
    const handleApplyFilters = () => {
        ...
        if (filterForm.credentialName) filters.credentialName = filterForm.credentialName;
    ...
    return (
        <div className='h-full bg-secondary flex flex-col'>
            ...
            <div className='bg-white px-3 py-4 border-b border-slate-200'>
                <div className='w-full'>
                    ...
                    <p className='mt-2 text-sm text-slate-600 leading-relaxed'>
                        Securely store, manage, and govern enterprise credentials including API keys, tokens, and authentication secrets.
                    </p>
                </div>
            </div>
                    ...
                    <div className='flex items-center gap-3 flex-wrap'>
                        ...
                        >
                            ...
                            <span className='text-sm'>
                                {isLoading ? 'Loading...' : 'Create New Credential'}
                            </span>
                        </button>
                                        ...
                                        <div className='space-y-2'>
                                            ...
                                            <div>
                                                ...
                                                <div className='relative'>
                                                    <input
                                                        type='text'
                                                        value={filterForm.credentialName}
                                                        onChange={(e) => {
                                                            ...
                                                            const filtered = (dropdownOptions.credentialNames || []).filter(credentialName =>
                                                                credentialName.name.toLowerCase().includes(value.toLowerCase())
                                                            );
                                                        ...
                                                        onFocus={() => {
                                                            if (filterForm.credentialName && filteredGroupNames.length > 0) {
                                                    ...
                                                    {showGroupNameSuggestions && (
                                                        <div className='filter-suggestions-dropdown absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-40 overflow-y-auto'>
                                                            {filteredGroupNames.map((credentialName, index) => (
                                                                <div
                                                                    key={credentialName.id}
                                                                    ...
                                                                    onMouseDown={(e) => {
                                                                        ...
                                                                        setFilterForm({
                                                                            ...filterForm,
                                                                            credentialName: credentialName.name,
                                                                        });
                                                                        setShowGroupNameSuggestions(false);
                                                                        setSelectedGroupNameIndex(-1);
                                                                    }}
                        ...
                        {isLoading ? (
                            ...
                            <div className='bg-white rounded-lg border border-slate-200 p-12 text-center'>
                                <div className='mx-auto max-w-md'>
                                    ...
                                    <h3 className='mt-4 text-lg font-semibold text-slate-900'>
                                        Loading Manage Credentials configurations
                                    </h3>
                            ...
                            <div className='bg-white rounded-lg border border-slate-200 p-12 text-center'>
                                <div className='mx-auto max-w-md'>
                                    ...
                                    <p className='mt-2 text-sm text-slate-500'>
                                        No credentials have been created yet for the selected Account and Enterprise combination. Create a new credential to get started.
                                    </p>
                                    <div className='mt-6'>
                                        <button
                                            onClick={handleAddNewRow}
                            ...
                            <div className='flex-1 overflow-auto'>
                                <ManageCredentialsTable
                                    ...
                                    onGroupByChange={(g: string) => {
                                        setActiveGroupLabel(
                                            g === 'credentialName' ? 'Credential Name' :
            ...
            {selectedRowForConnector && (() => {
                ...
                const latestRow = credentials.find(row => row.id === selectedRowForConnector.id) || selectedRowForConnector;
                ...
                return (
                    <ConnectorModal
                        ...
                        onSaveIndividual={(connectors: Connector[]) => {
                            ...
                            console.log('💾 [Credentials Page] Saving individual connector:', connectors);
                        ...
                        initialConnectors={(() => {
                            ...
                            console.log('📥 [Credentials Page] Loading initialConnectors for row:', latestRow.id, 'connectors:', savedConnectors.length);
