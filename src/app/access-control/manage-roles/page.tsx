...
import ManageUserRolesTable, { UserRoleRow } from '@/components/ManageUserRolesTable';
...
export default function ManageUserRoles() {
    ...
    const sortConfigsByDisplayOrder = useCallback((configs: UserRoleRow[]) => {
    ...
    useEffect(() => {
        console.log('🐛 [ManageuserRoles Page] State updated:', {
            selectedEnterprise,
    ...
    const userRolesRef = useRef<UserRoleRow[]>([]);
    ...
    // Update ref to track current userRoles state
    useEffect(() => {
    ...
    const [dropdownOptions, setDropdownOptions] = useState({
        roleNames: [] as Array<{id: string; name: string}>,
    ...
    const loadDropdownOptions = useCallback(async () => {
        try {
            ...
            // Extract unique role names from current User Roles
            const uniqueRoleNames = Array.from(new Set(userRoles.map(role => role.roleName)
                .filter(Boolean)
            )).map((name, index) => ({
                id: `roleName-${name}-${index}`,
                name: name
            }));

            // Extract unique entities from current User Roles
            const uniqueEntities = Array.from(new Set(userRoles.map(role => role.entity)
                .filter(Boolean)
            )).map((name, index) => ({
            ...
            setDropdownOptions({
                roleNames: [...uniqueRoleNames, ...(rolesRes?.map((r: any) => ({
                    id: r.id || r.roleId || String(Math.random()),
    ...
    // Process User Role data with filtering, sorting, and search
    const processedConfigs = React.useMemo(() => {
        ...
        if (appliedSearchTerm.trim()) {
            filtered = filtered.filter((config) => {
                ...
                return (
                    config.roleName?.toLowerCase().includes(searchLower) ||
        ...
        // Apply filters
        if (activeFilters.roleName) {
            filtered = filtered.filter(
                (config) => config.roleName?.toLowerCase().includes(activeFilters.roleName.toLowerCase()),
            );
        }
        ...
        if (sortColumn && sortDirection && (sortDirection === 'asc' || sortDirection === 'desc')) {
            filtered.sort((a, b) => {
                ...
                switch (sortColumn) {
                    case 'roleName':
                        valueA = (a.roleName || '').toString().toLowerCase();
                        valueB = (b.roleName || '').toString().toLowerCase();
                        break;
                    case 'description':
        ...
        try {
            ...
            if (response && typeof response === 'object' && 'data' in response) {
                ...
                if (rolesData && typeof rolesData === 'object' && 'roles' in rolesData) {
                    ...
                    console.log('🔄 [API Processing] Extracted roles property:', rolesData);
                }
            }

            console.log('🔄 [API Processing] Final rolesData:', rolesData);
            console.log('🔄 [API Processing] rolesData is array?', Array.isArray(rolesData));
            console.log('🔄 [API Processing] rolesData length:', Array.isArray(rolesData) ? rolesData.length : 'N/A');
            ...
            if (rolesData && Array.isArray(rolesData)) {
                ...
                const formattedRoles: UserRoleRow[] = rolesData.map((role: any, index: number) => {
                    ...
                    console.log(`✅ [API Processing] Formatted role ${index}:`, newRole);

                    // Track display order
                    displayOrderRef.current.set(newRole.id, index);

                    return newRole;
                });

                console.log('✅ [API Processing] Setting User Roles:', formattedRoles);
                setUserRoles(formattedRoles);
            } else {
                console.log('❌ [API Processing] No valid roles data found - setting empty array');
                setUserRoles([]);
            }
        } catch (error) {
            ...
            if (error && typeof error === 'object') {
                ...
                if (err.response?.status === 500) {
                    errorMessage = 'Server error: Unable to load User Roles. Please try again or contact support.';
                } else if (err.response?.status === 404) {
                    errorMessage = 'No User Roles found for the selected Account and Enterprise combination.';
                } else if (err.response?.status === 403) {
                    errorMessage = 'Access denied: You do not have permission to view User Roles for this Account and Enterprise.';
                } else if (err.message) {
    ...
    // Load User Roles on mount and whenever the selected account/enterprise changes.
    ...
    useEffect(() => {
        ...
        if (!isInitialized) {
            console.log('🔄 [ManageUserRoles] Waiting for initialization...');
            return;
        }
        ...
        console.log('🔄 [ManageUserRoles] Loading User Roles with context:', {
            selectedAccountId,
            selectedAccountName,
            selectedEnterprise,
            enterpriseId,
    ...
    const hasBlankRow = () => {
        return userRoles.some((role) => {
            const isTemporary = String(role.id).startsWith('tmp-');
            const isEmpty =
                !role.roleName &&
                !role.entity &&
                !role.product &&
    ...
    const validateIncompleteRows = () => {
        ...
        const incompleteTemporaryRows = temporaryRows.filter((role: any) => {
            ...
            const isCompletelyBlank = !hasroleName && !hasEntity && !hasProduct && !hasService;
        ...
        const incompleteExistingRows = existingRows.filter((role: any) => {
            const hasroleName = role.roleName?.trim();
            const hasEntity = role.entity?.trim();
            const hasProduct = role.product?.trim();
            const hasService = role.service?.trim();
        ...
        try {
            ...
            if (!String(pendingDeleteRowId).startsWith('tmp-')) {
                try {
                    ...
                    console.error('❌ Error deleting User Role from database:', error);
                    throw new Error('Failed to delete User Role from database');
                }
            } else {
    ...
    const autoSaveNewUserGroup = async (tempRowId: string, updatedGroup?: any) => {
        ...
        // Use the provided updated role or find it from current ref state
        const role = updatedGroup || userRolesRef.current.find((g) => g.id === tempRowId);
        if (!role) {
            console.error('❌ User Role not found for auto-save:', tempRowId);
            setSavingRows((prev) => {
        ...
        console.log('💾 Auto-saving new User Role:', role);

        // Check for duplicate entry (same roleName + entity + product + service)
        const isDuplicate = userRolesRef.current.some((existingRole) => {
            ...
            return existingRole.roleName?.toLowerCase().trim() === role.roleName?.toLowerCase().trim() &&
                   ...
                   existingRole.product?.toLowerCase().trim() === role.product?.toLowerCase().trim() &&
                   existingRole.service?.toLowerCase().trim() === role.service?.toLowerCase().trim();
        });
        ...
        console.log('🔍 [AutoSave] Checking if role exists in database:', role.roleName);
        const checkQueryParams = new URLSearchParams();
        checkQueryParams.append('name', role.roleName);
        ...
        try {
            ...
            if (exactMatch) {
                ...
                if (!savedRole?.id) {
                    console.error('⚠️ [AutoSave] WARNING: API did not return an ID. Response:', savedRole);
                    showBlueNotification('Failed to save User Role - API did not return a valid ID. Please try again.', 5000, false);
                    setSavingRows((prev) => {
                        const newSet = new Set(prev);
                        newSet.delete(tempRowId);
                        return newSet;
                ...
                setUserRoles((prev) => {
                    const updated = prev.map((g) =>
                        g.id === tempRowId
                            ? {...g, id: savedRoleId, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString()}
                            : g,
                    );
                    // Apply stable sorting to maintain display order
                    return sortConfigsByDisplayOrder(updated);
                ...
                if (oldDisplayOrder !== undefined) {
                    displayOrderRef.current.delete(tempRowId); // Remove old reference
                    displayOrderRef.current.set(savedRoleId, oldDisplayOrder); // Add new reference
                    console.log(`📍 [AutoSave] Preserved display order ${oldDisplayOrder} for new User Role ID ${savedRoleId}`);
                }
                ...
                const temporaryRows = userRolesRef.current.filter((role) => {
                    ...
                    if (isTemp && !isComplete) {
                        console.log(`🚫 Skipping incomplete temporary User Role ${role.id}:`, {
                            ...
                            groupNameValue: role.roleName,
                            entityValue: role.entity,
            ...
            if (totalRowsToSave > 0) {
                ...
                for (const tempRow of temporaryRows) {
                    ...
                    try {
                        ...
                        console.error(`❌ Failed to auto-save new User Role ${tempRow.id}:`, error);
                ...
                for (const modifiedRow of modifiedRows) {
                    ...
                    try {
                        ...
                        console.error(`❌ Failed to save modified User Role ${modifiedRow.id}:`, error);
                        failureCount++;
                    }
                }
                ...
                if (duplicateDetectedRef.current && successCount === 0 && failureCount === 0) {
                    ...
                    console.log('✅ Reload complete after autosave - checking state:', {
                        ...
                        userGroupsCount: userRolesRef.current.length,
                        hasTempRows: userRolesRef.current.some(g => String(g.id).startsWith('tmp-'))
                    });
                } else if (successCount > 0 && failureCount > 0 && !duplicateDetectedRef.current) {
    ...
    }, [selectedAccountId, selectedAccountName, selectedEnterpriseId, selectedEnterprise, showValidationErrors, loadUserRoles]);
    ...
    const getUnsavedChanges = () => {
        ...
        console.log('🔍 [getUnsavedChanges] Check:', {
            ...
            hasTempRows,
            tempRowsIds: userRolesRef.current.filter(g => String(g.id).startsWith('tmp-')).map(g => g.id),
    ...
    const getIncompleteRows = () => {
        const incompleteRows = userRoles
            .filter((role: any) => {
                const hasroleName = role.roleName?.trim();
                const hasEntity = role.entity?.trim();
                const hasProduct = role.product?.trim();
    ...
    const handleSaveAll = async () => {
        ...
        try {
            let savedCount = 0;
            const completeTemporaryRows = temporaryRows.filter((role: any) => {
                const hasroleName = role.roleName?.trim();
                const hasEntity = role.entity?.trim();
                const hasProduct = role.product?.trim();
                const hasService = role.service?.trim();
            ...
            for (const tempGroup of completeTemporaryRows) {
                try {
                    ...
                    queryParams.append('name', tempGroup.roleName);
                    ...
                    console.error('❌ Failed to save new User Role:', error);
                    failedCount++;
                }
            }
            ...
            for (const modifiedRole of completeModifiedRows) {
                try {
                    ...
                    const roleData = {
                        ...
                        entity: modifiedRole.entity,
                        product: modifiedRole.product,
    ...
    const handleApplyFilters = () => {
        ...
        if (filterForm.roleName) filters.roleName = filterForm.roleName;
            ...
            <div className='bg-white px-3 py-4 border-b border-slate-200'>
                <div className='w-full'>
                    ...
                    <p className='mt-2 text-sm text-slate-600 leading-relaxed'>
                        Create, organize, and govern User Roles through a unified access and permission framework.
                    </p>
                </div>
            </div>
                    ...
                    <div className='flex items-center gap-3 flex-wrap'>
                        ...
                        >
                            ...
                            <span className='text-sm'>
                                {isLoading ? 'Loading...' : 'Create New User Role'}
                            </span>
                        </button>
                                            ...
                                            <div>
                                                ...
                                                <div className='relative'>
                                                    <input
                                                        ...
                                                        onFocus={() => {
                                                            if (filterForm.roleName && filteredGroupNames.length > 0) {
                                                    ...
                                                    {showGroupNameSuggestions && (
                                                        <div className='filter-suggestions-dropdown absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-40 overflow-y-auto'>
                                                            {filteredGroupNames.map((roleName, index) => (
                                                                <div
                                                                    key={roleName.id}
                        ...
                        {isLoading ? (
                            ...
                            <div className='bg-white rounded-lg border border-slate-200 p-12 text-center'>
                                <div className='mx-auto max-w-md'>
                                    ...
                                    <h3 className='mt-4 text-lg font-semibold text-slate-900'>
                                        Loading Manage User Roles configurations
                                    </h3>
                            ...
                            <div className='bg-white rounded-lg border border-slate-200 p-12 text-center'>
                                <div className='mx-auto max-w-md'>
                                    ...
                                    <p className='mt-2 text-sm text-slate-500'>
                                        No User Roles have been created yet for the selected Account and Enterprise combination. Create a new User Role to get started.
                                    </p>
                                    <div className='mt-6'>
                                        <button
                                            onClick={handleAddNewRow}
