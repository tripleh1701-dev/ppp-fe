 ...
 * ============================================================================
 * 
 * This file currently uses localStorage to persist builds data for testing.
 * 
 ...
 * 6. Update loadBuilds function to call actual API endpoints
 * 7. Update all setBuilds calls to use API save functions instead
 * 
 * The localStorage key used is: 'builds_builds_data'
 * ============================================================================
 */
...
import IntegrationArtifactsModal from '@/components/IntegrationArtifactsModal';
...
export default function Integrations() {
    ...
    // ============================================================================
    // This section stores builds data in localStorage for testing purposes.
    ...
    const LOCAL_STORAGE_BUILDS_KEY = 'builds_builds_data';
    ...
    const getLocalStorageKey = (accountId: string, enterpriseId: string): string => {
        if (accountId && enterpriseId) {
            return `${LOCAL_STORAGE_BUILDS_KEY}_${accountId}_${enterpriseId}`;
        }
        // Fallback to default key if no account/enterprise
        return LOCAL_STORAGE_BUILDS_KEY;
    };
    
    // Load builds from localStorage for current account/enterprise
    const loadBuildsFromLocalStorage = useCallback((accountId?: string, enterpriseId?: string): BuildRow[] => {
        ...
        try {
            ...
            if (stored) {
                ...
                console.log('📦 [LocalStorage] Loaded builds from localStorage:', parsed.length, 'rows for account/enterprise:', accountId, enterpriseId);
                return parsed;
            }
        } catch (error) {
            console.error('❌ [LocalStorage] Failed to load builds from localStorage:', error);
        }
        return [];
    }, []);
    
    // Save builds to localStorage for current account/enterprise
    const saveBuildsToLocalStorage = useCallback((roles: BuildRow[], accountId?: string, enterpriseId?: string) => {
        if (!USE_LOCAL_STORAGE || typeof window === 'undefined') return;
        try {
            if (!accountId || !enterpriseId) {
            ...
            console.log('💾 [LocalStorage] Saved builds to localStorage:', roles.length, 'rows for account/enterprise:', accountId, enterpriseId);
        } catch (error) {
            console.error('❌ [LocalStorage] Failed to save builds to localStorage:', error);
        }
    }, []);
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            (window as any).checkBuildsLocalStorage = () => {
                try {
                    const stored = window.localStorage.getItem(LOCAL_STORAGE_BUILDS_KEY);
                    if (stored) {
                        const parsed = JSON.parse(stored);
                        console.log('📦 [Debug] Builds in localStorage:', parsed);
                        console.log('📦 [Debug] Count:', parsed.length, 'rows');
                        return parsed;
                    } else {
                        console.log('📦 [Debug] No builds data in localStorage');
                        return [];
                    }
                } catch (error) {
            ...
            if (stored) {
                try {
                    const parsed = JSON.parse(stored);
                    console.log('📦 [LocalStorage] Found existing builds data:', parsed.length, 'rows');
                } catch (error) {
                    console.error('❌ [LocalStorage] Error parsing existing data:', error);
                }
            } else {
                console.log('📦 [LocalStorage] No existing builds data found');
            }
        }
    }, []);
    // ============================================================================
    ...
    const sortBuildsByDisplayOrder = useCallback((configs: BuildRow[]) => {
    ...
    const [selectedRowForConnector, setSelectedRowForConnector] = useState<BuildRow | null>(null);
    const [isIntegrationArtifactsModalOpen, setIsIntegrationArtifactsModalOpen] = useState(false);
    const [selectedRowForIntegrationArtifacts, setSelectedRowForIntegrationArtifacts] = useState<BuildRow | null>(null);
    ...
    useEffect(() => {
        ...
        const checkOAuthCompletion = () => {
            ...
            if (shouldReopenModal === 'true' && connectorId && connectorName) {
                ...
                const matchingRow = builds.find(row => row.connectorName === connectorName);
                
                if (matchingRow) {
                    console.log('🔄 [OAuth] Reopening modal for build:', connectorName);
    ...
    useEffect(() => {
        const loadSelectedEnterprise = () => {
            try {
                console.log('🐛 [ManageBuilds Page] Loading localStorage values...');
                ...
                console.log('🐛 [ManageBuilds Page] localStorage values:', {
                ...
                console.log('🐛 [ManageBuilds Page] Setting state values:', {
                    enterprise: savedName || '',
    ...
    useEffect(() => {
        console.log('🐛 [ManageBuilds Page] State updated:', {
            selectedEnterprise,
    ...
    // Track previous builds state to prevent unnecessary saves
    const prevBuildsRef = useRef<string>('');
    
    // Auto-save builds to localStorage whenever it changes (but not during loading)
    useEffect(() => {
        ...
        // Only save if we have account/enterprise selected and builds has data
        if (USE_LOCAL_STORAGE && selectedAccountId && selectedEnterpriseId && builds.length > 0) {
            // Deep comparison to check if builds actually changed
            const currentBuildsStr = JSON.stringify(builds);
            if (currentBuildsStr === prevBuildsRef.current) {
                // No actual change, skip save
                return;
            }
            
            // Use a longer delay to batch multiple rapid updates and prevent excessive saves
            const saveTimer = setTimeout(() => {
                if (!isLoadingRef.current) { // Double-check loading flag
                    // Double-check that builds actually changed before saving (state might have changed back)
                    const finalBuildsStr = JSON.stringify(builds);
                    if (finalBuildsStr === prevBuildsRef.current) {
                        // State changed back or was updated by another effect, skip save
                        return;
                    }
                    
                    // Update ref AFTER confirming we're going to save
                    prevBuildsRef.current = finalBuildsStr;
                    
                    // Only log if there are actual pipeline stages changes (reduce verbosity)
                    const rowsWithPipelineStages = builds.filter(b => b.pipelineStagesState);
                    if (rowsWithPipelineStages.length > 0) {
                        // Only log once per save, not for every row
                        console.log('💾 [AutoSave] Saving', builds.length, 'builds (', rowsWithPipelineStages.length, 'with pipeline stages)');
                    } else {
                        console.log('💾 [AutoSave] Saving', builds.length, 'builds to localStorage');
                    }
                    
                    saveBuildsToLocalStorage(builds, selectedAccountId, selectedEnterpriseId);
                }
            }, 1000); // Increased delay to 1000ms to batch more updates together
            
            return () => clearTimeout(saveTimer);
        }
    }, [builds, selectedAccountId, selectedEnterpriseId, isInitialized, saveBuildsToLocalStorage]);
    ...
    // BuildsPanel state
    const [isBuildsPanelOpen, setIsBuildsPanelOpen] = useState(false);
    
    // BuildSlidingPanels state
    const [isBuildSlidingPanelsOpen, setIsBuildSlidingPanelsOpen] = useState(false);
    const [selectedBuildRowForSlidingPanels, setSelectedBuildRowForSlidingPanels] = useState<BuildRow | null>(null);
    
    // BuildDetailPanel state
    const [isBuildDetailPanelOpen, setIsBuildDetailPanelOpen] = useState(false);
    const [selectedBuildRowForDetailPanel, setSelectedBuildRowForDetailPanel] = useState<BuildRow | null>(null);
    
    // BuildDetailModal state
    const [isBuildDetailModalOpen, setIsBuildDetailModalOpen] = useState(false);
    const [selectedBuildRowForModal, setSelectedBuildRowForModal] = useState<BuildRow | null>(null);
    
    // Split view state
    const [splitViewWidth, setSplitViewWidth] = useState(50); // Percentage for left panel (table)
    ...
    const [ActiveGroupLabel, setActiveGroupLabel] = useState<
        'None' | 'Job Name' | 'Description' | 'Workstream' | 'Pipeline'
    >('None');
    
    type ColumnType = 'connectorName' | 'description' | 'entity' | 'pipeline' | 'status' | 'scope' | 'connectivityStatus' | 'actions';
    ...
    // NOTE: API calls removed - builds should use build-specific APIs instead
    const loadDropdownOptions = useCallback(async () => {
        try {
            ...
            console.log('⚠️ [Builds] loadDropdownOptions called but APIs removed - use build APIs instead');

            // Extract unique connector names from current Builds (local state only)
            const uniqueConnectorNames = Array.from(new Set(builds.map(build => build.connectorName)
                .filter(Boolean)
            )).map((name, index) => ({
            ...
            // Extract unique entities from current Builds (local state only)
            const uniqueEntities = Array.from(new Set(builds.map(build => build.entity)
                .filter(Boolean)
            )).map((name, index) => ({
    ...
    const allCols: ColumnType[] = ['connectorName', 'description', 'entity', 'pipeline', 'status', 'scope', 'connectivityStatus'];
    
    // Columns available for sorting
    const sortableCols: ColumnType[] = ['connectorName', 'description', 'entity', 'pipeline'];
    ...
    // Load Builds from database. Accept optional filters so we can request groups
    ...
    const loadBuilds = useCallback(async (filters?: {
        ...
        try {
            ...
            if (USE_LOCAL_STORAGE) {
                const loadedRoles = loadBuildsFromLocalStorage();
                setBuilds(loadedRoles);
                console.log('📦 [Builds] Loaded from localStorage:', loadedRoles.length, 'rows');
            } else {
                // TODO: Replace with actual API call when backend is ready
                // API call removed - connectors should use connector-specific APIs
                // Original API: /api/user-management/roles
                console.log('⚠️ [Builds] loadBuilds called but API removed - use build APIs instead');
                setBuilds([]);
            }
            // ============================================================================
        } catch (error) {
    ...
    useEffect(() => {
        if (USE_LOCAL_STORAGE && isInitialized && selectedAccountId && selectedEnterpriseId) {
            ...
            const loadedRoles = loadBuildsFromLocalStorage(selectedAccountId, selectedEnterpriseId);
            if (loadedRoles.length > 0) {
                // Log pipelineStagesState for debugging
                const rowsWithPipelineStages = loadedRoles.filter(b => b.pipelineStagesState);
                if (rowsWithPipelineStages.length > 0) {
                    console.log('📦 [Builds] Initial load - builds with pipelineStagesState:', rowsWithPipelineStages.length, 'rows');
                    rowsWithPipelineStages.forEach(row => {
                        console.log('📦 [Builds] Row', row.id, 'has pipelineStagesState:', row.pipelineStagesState);
                    });
                }
                setBuilds(loadedRoles);
                console.log('📦 [Builds] Initial load from localStorage:', loadedRoles.length, 'rows for account:', selectedAccountId, 'enterprise:', selectedEnterpriseId);
            }
            // Reset loading flag after a short delay to allow state to update
            setTimeout(() => {
                isLoadingRef.current = false;
    ...
    }, [isInitialized, selectedAccountId, selectedEnterpriseId, loadBuildsFromLocalStorage]);
    
    // Load Builds on mount and whenever the selected account/enterprise changes.
    ...
    useEffect(() => {
        ...
        if (!isInitialized) {
            console.log('🔄 [ManageBuilds] Waiting for initialization...');
            return;
        }
        ...
        console.log('🔄 [ManageBuilds] Loading Builds with context:', {
            selectedAccountId,
            selectedAccountName,
            selectedEnterprise,
            enterpriseId,
        ...
        if (selectedAccountId && currentEnterpriseId) {
            ...
            // ============================================================================
            if (USE_LOCAL_STORAGE) {
                const loadedRoles = loadBuildsFromLocalStorage(selectedAccountId, currentEnterpriseId);
                // Log pipelineStagesState for debugging
                const rowsWithPipelineStages = loadedRoles.filter(b => b.pipelineStagesState);
                if (rowsWithPipelineStages.length > 0) {
                    console.log('📦 [Builds] Loaded builds with pipelineStagesState:', rowsWithPipelineStages.length, 'rows');
                    rowsWithPipelineStages.forEach(row => {
                        console.log('📦 [Builds] Row', row.id, 'has pipelineStagesState:', row.pipelineStagesState);
                    });
                }
                setBuilds(loadedRoles);
                console.log('📦 [Builds] Loaded from localStorage:', loadedRoles.length, 'rows for account:', selectedAccountId, 'enterprise:', currentEnterpriseId);
            } else {
                console.log('⚠️ [Builds] loadBuilds call removed - use build APIs instead');
                // loadBuilds({...}); // REMOVED - use build-specific APIs
            }
            // ============================================================================
            setIsLoading(false);
            // Reset loading flag after a short delay to allow state to update
        ...
        if (!selectedAccountId) {
            showBlueNotification('Please select an Account from the top-right dropdown to view Builds', 5000, false);
        }
        // Enterprise notification removed - enterprise is now auto-selected based on account licenses
    }, [selectedAccountId, selectedAccountName, selectedEnterprise, selectedEnterpriseId, isInitialized, loadBuilds, loadBuildsFromLocalStorage]);
    
    // Load dropdown options whenever builds changes - use a ref to prevent infinite loops
    const dropdownOptionsLoadedRef = useRef(false);
    const buildsCountRef = useRef(0);
    useEffect(() => {
        if (!isLoading && builds.length > 0 && 
            (builds.length !== buildsCountRef.current || !dropdownOptionsLoadedRef.current)) {
            buildsCountRef.current = builds.length;
            dropdownOptionsLoadedRef.current = true;
            loadDropdownOptions();
        }
    }, [builds.length, loadDropdownOptions, isLoading]);
    ...
    const hasBlankRow = () => {
        return builds.some((build) => {
            const isTemporary = String(build.id).startsWith('tmp-');
            const isEmpty =
                !build.connectorName &&
                !build.entity &&
                !build.product &&
    ...
    const validateIncompleteRows = () => {
        ...
        const temporaryRows = builds.filter((build: any) => 
            String(build.id).startsWith('tmp-')
        );

        // Get all existing rows
        const existingRows = builds.filter((build: any) => 
            !String(build.id).startsWith('tmp-')
        );

        // Check for incomplete temporary rows (exclude completely blank rows)
        const incompleteTemporaryRows = temporaryRows.filter((build: any) => {
            const hasconnectorName = build.connectorName?.trim();
            const hasEntity = build.entity?.trim();
            const hasPipeline = build.pipeline?.trim();
            const hasProduct = build.product?.trim();
            const hasService = build.service?.trim();

            // Don't include completely blank rows (new rows that haven't been touched)
            const isCompletelyBlank = !hasconnectorName && !hasEntity && !hasPipeline && !hasProduct && !hasService;
            if (isCompletelyBlank) return false;

            // Row is incomplete if it has some data but not all required fields
            return !hasconnectorName || !hasEntity || !hasPipeline || !hasProduct || !hasService;
        });

        // Check for incomplete existing rows
        const incompleteExistingRows = existingRows.filter((build: any) => {
            const hasconnectorName = build.connectorName?.trim();
            const hasEntity = build.entity?.trim();
            const hasPipeline = build.pipeline?.trim();
            const hasProduct = build.product?.trim();
        ...
        if (incompleteRows.length > 0) {
            ...
            incompleteRows.forEach((build) => {
                if (!build.connectorName?.trim()) missingFields.add('Job Name');
                if (!build.entity?.trim()) missingFields.add('Workstream');
                if (!build.pipeline?.trim()) missingFields.add('Pipeline Name');
                if (!build.product?.trim()) missingFields.add('Product');
                if (!build.service?.trim()) missingFields.add('Service');
            });
    ...
    // Event listener for opening BuildDetailPanel from "+" button
    useEffect(() => {
        const handleOpenBuildsPanel = () => {
            console.log('🔧 [Builds] openBuildsPanel event received');
            setIsBuildDetailPanelOpen(true);
            setSelectedBuildRowForDetailPanel(null); // Create new build
        };

        if (typeof window !== 'undefined') {
            window.addEventListener('openBuildsPanel', handleOpenBuildsPanel);
            return () => {
                window.removeEventListener('openBuildsPanel', handleOpenBuildsPanel);
            };
        }
    }, []);
    ...
    const handleAddNewRow = () => {
        ...
        const newRole: BuildRow = {
            id: `tmp-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
            connectorName: '',
            description: '',
            entity: '',
            product: 'DevOps', // Fixed value
            service: 'Integration', // Fixed value
            scope: '',
    ...
    const handleUpdateField = useCallback((rowId: string, field: string, value: any) => {
        ...
        // Special handling for pipelineStagesState to ensure it's properly saved
        if (field === 'pipelineStagesState') {
            console.log('💾 [handleUpdateField] Saving pipelineStagesState for row:', rowId, value);
        }
        
        // First, update the state
        let updatedGroup: BuildRow | null = null;
        setBuilds(prev => {
            ...
            return prev.map(build => {
                if (build.id === rowId) {
                    ...
                    // Ensure Product and Service are always set to fixed values
                    updatedGroup = { 
                        ...build, 
                        [field]: value,
                        product: 'DevOps', // Always set to fixed value
                        service: 'Integration', // Always set to fixed value
                    };
                    
                    // Log pipelineStagesState updates for debugging
                    if (field === 'pipelineStagesState') {
                        console.log('💾 [handleUpdateField] Updated build row with pipelineStagesState:', {
                            rowId,
                            hasPipelineStagesState: !!updatedGroup.pipelineStagesState,
                            pipelineStagesState: updatedGroup.pipelineStagesState
                        });
                    }
                    
                    return updatedGroup;
                }
                return build; // Return same reference for unchanged rows
            });
        });
        ...
        if (updatedGroup) {
            ...
            const hasPipeline = (updatedGroup as any).pipeline?.trim() && (updatedGroup as any).pipeline.trim().length > 0;
            ...
            const isComplete = hasconnectorName && hasEntity && hasPipeline && hasProduct && hasService;
    ...
    const confirmDelete = async () => {
        ...
        setDeletingRow(true);
        try {
            console.log('🗑️ Deleting Build:', pendingDeleteRowId);
            
            // Add a small delay to show the loading state
            await new Promise(resolve => setTimeout(resolve, 1000));
            
            // Find the build to be deleted for debugging
            const buildToDelete = builds.find(g => g.id === pendingDeleteRowId);
            console.log('📄 Build data to delete:', buildToDelete);
            ...
            if (!String(pendingDeleteRowId).startsWith('tmp-')) {
                try {
                    ...
                    console.log('⚠️ [Builds] Delete API removed - use build APIs instead');
                    console.log('🗑️ Would delete build with ID:', pendingDeleteRowId);
                ...
                // Temporary rows are handled by the state update below, which will trigger auto-save
            }

            // Remove from local state
            let updatedBuilds: BuildRow[] = [];
            setBuilds((prev) => {
                updatedBuilds = prev.filter((build) => build.id !== pendingDeleteRowId);
                // Apply stable sorting to maintain display order
                return sortBuildsByDisplayOrder(updatedBuilds);
            });

            // Save updated builds to localStorage immediately after deletion
            if (USE_LOCAL_STORAGE && selectedAccountId && selectedEnterpriseId) {
                try {
                    saveBuildsToLocalStorage(updatedBuilds, selectedAccountId, selectedEnterpriseId);
                    console.log('💾 [Delete] Saved updated builds to localStorage after deletion:', updatedBuilds.length, 'rows');
                } catch (error) {
                    console.error('❌ [Delete] Error saving to localStorage after deletion:', error);
                }
            }
            ...
            showBlueNotification('Failed to delete Build. Please try again.', 5000, false);
        } finally {
            setDeletingRow(false);
        }
    };

    // Auto-save new Build when all required fields are filled - exactly like Manage Users
    const autoSaveNewBuild = async (tempRowId: string, updatedGroup?: any) => {
        console.log('🚀 autoSaveNewBuild function called with tempRowId:', tempRowId);

        // Mark row as saving
        setSavingRows((prev) => new Set([...Array.from(prev), tempRowId]));

        // Use the provided updated build or find it from current ref state
        const build = updatedGroup || buildsRef.current.find((g) => g.id === tempRowId);
        if (!build) {
            console.error('❌ Build not found for auto-save:', tempRowId);
            setSavingRows((prev) => {
        ...
        console.log('💾 Auto-saving new Build:', build);

        // Check for duplicate entry (same connectorName + entity + product + service)
        const isDuplicate = buildsRef.current.some((existingBuild) => {
            // Skip the current temporary row being saved
            if (existingBuild.id === tempRowId) return false;
            
            // Check if all key fields match
            return existingBuild.connectorName?.toLowerCase().trim() === build.connectorName?.toLowerCase().trim() &&
                   existingBuild.entity?.toLowerCase().trim() === build.entity?.toLowerCase().trim() &&
                   existingBuild.product?.toLowerCase().trim() === build.product?.toLowerCase().trim() &&
                   existingBuild.service?.toLowerCase().trim() === build.service?.toLowerCase().trim();
        });

        if (isDuplicate) {
            console.error('❌ Duplicate entry detected - Build with same Job Name, Workstream, Product, and Service already exists');
            ...
            setDuplicateMessage(
                `This combination of Job Name (${build.connectorName}), Workstream (${build.entity}), Product (${build.product}), and Service (${build.service}) already exists in another row. Please use a different combination.`
            );
        ...
        // Check if a build with this name already exists in the database (created via + Add button)
        // NOTE: API calls removed - builds should use build-specific APIs instead
        console.log('⚠️ [Builds] Auto-save API calls removed - use build APIs instead');
        console.log('🔍 [AutoSave] Would check if build exists:', build.connectorName);
        
        // API calls removed - builds should use build-specific APIs
        ...
        // Update the builds state with the new ID
        setBuilds((prev) => {
            ...
            // Apply stable sorting to maintain display order
            return sortBuildsByDisplayOrder(updated);
        });
        ...
        if (oldDisplayOrder !== undefined) {
            ...
            console.log(`📍 [AutoSave] Preserved display order ${oldDisplayOrder} for new build ID ${newId}`);
        }

        console.log('🎉 [AutoSave] Build updated locally (API calls removed)');
        ...
        const timer = setTimeout(async () => {
            try {
                ...
                const temporaryRows = buildsRef.current.filter((build) => {
                    const isTemp = String(build.id).startsWith('tmp-');
                    if (!isTemp) return false;
                    
                    // Be more strict about what constitutes a complete Build row
                    const hasconnectorName = build.connectorName?.trim() && build.connectorName.trim().length > 0;
                    const hasEntity = build.entity?.trim() && build.entity.trim().length > 0;
                    const hasPipeline = build.pipeline?.trim() && build.pipeline.trim().length > 0;
                    const hasProduct = build.product?.trim() && build.product.trim().length > 0;
                    const hasService = build.service?.trim() && build.service.trim().length > 0;
                    
                    const isComplete = hasconnectorName && hasEntity && hasPipeline && hasProduct && hasService;
                    
                    if (isTemp && !isComplete) {
                        console.log(`🚫 Skipping incomplete temporary Build ${build.id}:`, {
                            hasconnectorName: !!hasconnectorName,
                            hasEntity: !!hasEntity,
                            hasPipeline: !!hasPipeline,
                            hasProduct: !!hasProduct,
                            hasService: !!hasService,
                            groupNameValue: build.connectorName,
                            entityValue: build.entity,
                            pipelineValue: build.pipeline,
                            productValue: build.product,
                            serviceValue: build.service
                        });
                    }
                ...
                const modifiedRows = buildsRef.current.filter((build) => {
                    const isExisting = !String(build.id).startsWith('tmp-');
                    const isModified = modifiedExistingRecordsRef.current.has(String(build.id));
                    ...
                    if (isExisting && isModified) {
                        ...
                        console.log(`🔍 Checking modified Build ${build.id}: isComplete=${isComplete}`, {
                            hasconnectorName: !!hasconnectorName,
                            hasEntity: !!hasEntity,
                            hasPipeline: !!hasPipeline,
                            hasProduct: !!hasProduct,
                    ...
                    console.log(`🔍 Checking Build ${build.id}: isExisting=${isExisting}, isModified=${isModified}`);
                    return false;
                });
                
            console.log(`📊 Found ${temporaryRows.length} complete temporary Builds to auto-save`);
            console.log(`📊 Found ${modifiedRows.length} modified existing Builds to auto-save`);
            ...
            const orphanedRecords = Array.from(modifiedExistingRecordsRef.current).filter(recordId => 
                !buildsRef.current.find(build => String(build.id) === recordId)
            );
            ...
            if (totalRowsToSave > 0) {
                console.log('💾 Auto-saving Builds after 10 seconds of inactivity...', temporaryRows.map(r => r.id));
                
                let successCount = 0;
                let failureCount = 0;
                
                // Save new temporary Builds
                for (const tempRow of temporaryRows) {
                    console.log(`💾 Auto-saving Build: ${tempRow.id}`);
                    
                    // Reset duplicate flag before each save attempt
                    const duplicateFlagBefore = duplicateDetectedRef.current;
                    
                    try {
                        await autoSaveNewBuild(tempRow.id);
                ...
                // Save modified existing Builds to database via API
                for (const modifiedRow of modifiedRows) {
                    console.log(`💾 Saving modified existing Build: ${modifiedRow.id}`);
                    try {
                        // Check for duplicate entry (same connectorName + entity + product + service as another record)
                        const isDuplicate = buildsRef.current.some((existingRole) => {
                        ...
                        // NOTE: API call removed - builds should use build-specific APIs instead
                        // Original API: PUT /api/user-management/roles/:id
                        console.log('⚠️ [Builds] Update API call removed - use build APIs instead');
                        console.log(`💾 Would update build ${modifiedRow.id}`);
                        // await api.put(`/api/user-management/roles/${modifiedRow.id}`, {...}); // REMOVED
                        console.log(`✅ Build ${modifiedRow.id} updated locally (API calls removed)`);
                        successCount++;
                    } catch (error) {
                        console.error(`❌ Failed to save modified Build ${modifiedRow.id}:`, error);
                        failureCount++;
                    }
                }
                ...
                if (duplicateDetectedRef.current && successCount === 0 && failureCount === 0) {
                    ...
                    // Don't reload from localStorage after autosave - we've already updated the state
                    // The localStorage save happens via useEffect, so we just need to ensure it completes
                    // Wait a bit to ensure localStorage save completes before doing anything else
                    await new Promise(resolve => setTimeout(resolve, 200));
                    
                    console.log('✅ Auto-save complete - state updated, localStorage should be synced:', {
                        autoSaveTimerRef: autoSaveTimerRef.current,
                        modifiedRecordsSize: modifiedExistingRecordsRef.current.size,
                        buildsCount: buildsRef.current.length,
                        hasTempRows: buildsRef.current.some(g => String(g.id).startsWith('tmp-'))
                    });
                } else if (successCount > 0 && failureCount > 0 && !duplicateDetectedRef.current) {
    ...
    }, [selectedAccountId, selectedAccountName, selectedEnterpriseId, selectedEnterprise, showValidationErrors, loadBuilds]);
    ...
    const getUnsavedChanges = () => {
        ...
        const hasTempRows = buildsRef.current.some((build: any) => 
            String(build.id).startsWith('tmp-')
        );
        ...
        console.log('🔍 [getUnsavedChanges] Check:', {
            ...
            hasTempRows,
            tempRowsIds: buildsRef.current.filter(g => String(g.id).startsWith('tmp-')).map(g => g.id),
            totalBuilds: buildsRef.current.length,
            hasUnsavedChanges
        });
    ...
    const getIncompleteRows = () => {
        ...
        if (showValidationErrors && incompleteRows.length > 0) {
            console.log('🔍 getIncompleteRows result:', {
                ...
                sampleBuildIds: builds.slice(0, 3).map((g: BuildRow) => g.id)
            });
        }
    ...
    const handleSaveAll = async () => {
        console.log('💾 Save all clicked - validating and saving Builds...');
        ...
        const modifiedExistingRowsCount = existingRows.filter((build: any) => 
            modifiedExistingRecords.has(build.id)
        ).length;
        ...
        if (incompleteRowsData.length > 0) {
            ...
            incompleteRowsData.forEach((build) => {
                console.log('📋 Checking build:', {
                    id: build.id,
                    connectorName: build.connectorName || '(empty)',
                    entity: build.entity || '(empty)',
                    pipeline: build.pipeline || '(empty)',
                    product: build.product || '(empty)',
                    service: build.service || '(empty)'
                });
                
                // Check for missing fields
                if (!build.connectorName?.trim()) allMissingFields.add('Job Name');
                if (!build.entity?.trim()) allMissingFields.add('Workstream');
                if (!build.pipeline?.trim()) allMissingFields.add('Pipeline Name');
                if (!build.product?.trim()) allMissingFields.add('Product');
                if (!build.service?.trim()) allMissingFields.add('Service');
            });
        ...
        try {
            let savedCount = 0;
            const completeTemporaryRows = temporaryRows.filter((build: any) => {
                const hasconnectorName = build.connectorName?.trim();
                const hasEntity = build.entity?.trim();
                const hasPipeline = build.pipeline?.trim();
                const hasProduct = build.product?.trim();
            ...
            const completeModifiedRows = existingRows.filter((build: any) => {
                const hasconnectorName = build.connectorName?.trim();
                const hasEntity = build.entity?.trim();
                const hasPipeline = build.pipeline?.trim();
                const hasProduct = build.product?.trim();
                const hasService = build.service?.trim();
                const isModified = modifiedExistingRecords.has(build.id);
                return hasconnectorName && hasEntity && hasPipeline && hasProduct && hasService && isModified;
            });
            ...
            for (const tempGroup of completeTemporaryRows) {
                try {
                    // Check for duplicate entry (same connectorName + entity + product + service)
                    const isDuplicate = builds.some((existingRole: any) => {
                        // Skip the current temporary row being saved
                    ...
                    // Check if a build with this name already exists in the database (created via + Add button)
                    // NOTE: API calls removed - builds should use build-specific APIs instead
                    console.log('⚠️ [Builds] Save API calls removed - use build APIs instead');
                    console.log('🔍 Would check if build exists:', tempGroup.connectorName);
                    ...
                    savedCount++;
                    console.log('🎉 Build updated locally (API calls removed)');
                } catch (error) {
                    console.error('❌ Failed to save new Build:', error);
                    failedCount++;
                }
            }
            ...
            for (const modifiedRole of completeModifiedRows) {
                try {
                    ...
                    // NOTE: API call removed - builds should use build-specific APIs instead
                    console.log('⚠️ [Builds] Update API call removed - use build APIs instead');
                    console.log('💾 Would update existing build:', modifiedRole.id);
                    
                    // API call removed - builds should use build-specific APIs
                    // Original API: PUT /api/user-management/roles/:id
                ...
                if (USE_LOCAL_STORAGE && selectedAccountId && selectedEnterpriseId) {
                    ...
                    setTimeout(() => {
                        ...
                        if (currentBuilds.length > 0) {
                            // Log pipelineStagesState for debugging
                            const rowsWithPipelineStages = currentBuilds.filter(b => b.pipelineStagesState);
                            if (rowsWithPipelineStages.length > 0) {
                                console.log('💾 [Save] Saving builds with pipelineStagesState:', rowsWithPipelineStages.length, 'rows');
                                rowsWithPipelineStages.forEach(row => {
                                    console.log('💾 [Save] Row', row.id, 'has pipelineStagesState:', row.pipelineStagesState);
                                });
                            }
                            
                            // Save current state to localStorage
                            saveBuildsToLocalStorage(currentBuilds, selectedAccountId, selectedEnterpriseId);
                            console.log('💾 [Save] Forced localStorage save after manual save:', currentBuilds.length, 'rows');
                            console.log('💾 [Save] Saved row IDs:', currentBuilds.map((r: BuildRow) => r.id));
                        } else {
                            // If ref is empty, try to get from state directly
                            console.warn('⚠️ [Save] buildsRef is empty, checking state...');
                        }
                    }, 200);
                }
                // ============================================================================
    ...
    return (
        <div className="h-full bg-secondary flex flex-col">
            ...
            <div className='flex-1 p-3 overflow-hidden'>
                {(() => {
                    console.log('🔍 [Page] Split view check:', {
                        isBuildDetailPanelOpen,
                        hasSelectedRow: !!selectedBuildRowForDetailPanel,
                        selectedRowId: selectedBuildRowForDetailPanel?.id,
                        shouldShowSplitView: isBuildDetailPanelOpen && selectedBuildRowForDetailPanel
                    });
                    return isBuildDetailPanelOpen && selectedBuildRowForDetailPanel ? (
                        // Split View: Table and BuildDetailPanel side by side
                        ...
                        >
                            ...
                            <div className='bg-white rounded-lg border border-slate-200 p-12 text-center'>
                                <div className='mx-auto max-w-md'>
                                    ...
                                    <h3 className='mt-4 text-lg font-semibold text-slate-900'>
                                        Loading Manage Builds configurations
                                    </h3>
                            ...
                            <div className='bg-white rounded-lg border border-slate-200 p-12 text-center'>
                                <div className='mx-auto max-w-md'>
                                    ...
                                    <p className='mt-2 text-sm text-slate-500'>
                                        No builds have been created yet for the selected Account and Enterprise combination. Create a new job to get started.
                                    </p>
                                    <div className='mt-6'>
                                        <button
                                            onClick={handleAddNewRow}
                            ...
                            <div className='flex-1 overflow-auto'>
                                <BuildsTable
                                    ...
                                    onOpenScopeModal={(row: BuildRow) => {
                                        setSelectedRowForIntegrationArtifacts(row);
                                        setIsIntegrationArtifactsModalOpen(true);
                                    }}
                                    onOpenBuildDetail={(row: BuildRow) => {
                                        console.log('🟢 [Page] onOpenBuildDetail called with row:', row);
                                        setSelectedBuildRowForModal(row);
                                        setIsBuildDetailModalOpen(true);
                                        console.log('🟢 [Page] State set - isBuildDetailModalOpen: true, selectedBuildRowForModal:', row);
                                    }}
                                />
                            </div>
                                        )}
                            ...
                            {/* Resizable Separator */}
                            <div
                                className={`h-full w-1 bg-slate-300 cursor-col-resize hover:bg-blue-400 transition-colors flex-shrink-0 ${
                                    isDraggingSeparator ? 'bg-blue-500' : ''
                                }`}
                                onMouseDown={(e) => {
                                    e.preventDefault();
                                    ...
                                    const handleMouseMove = (moveEvent: MouseEvent) => {
                                        ...
                                        const newWidth = Math.max(20, Math.min(80, splitViewWidth + deltaPercent));
                            ...
                            {/* BuildDetailPanel Section */}
                            <div 
                                className='h-full overflow-hidden transition-all duration-200'
                                style={{ width: `${100 - splitViewWidth}%` }}
                            >
                                <div className='h-full space-y-3'>
                                    <div className='bg-white border border-light rounded-lg p-3 h-full flex flex-col'>
                                        <BuildDetailPanel
                                            isOpen={true}
                                            renderInline={true}
                                            buildRow={selectedBuildRowForDetailPanel}
                                            onClose={() => {
                                                setIsBuildDetailPanelOpen(false);
                                                setSelectedBuildRowForDetailPanel(null);
                                            }}
                                            sidebarWidth={isAIPanelCollapsed ? 0 : 256}
                                        />
                                    </div>
                                </div>
                            </div>
            ...
            {selectedRowForConnector && (() => {
                // Get the latest row from builds to ensure we have the most up-to-date data
                const latestRow = builds.find((row: BuildRow) => row.id === selectedRowForConnector?.id) || selectedRowForConnector;
                ...
                return (
                    <ConnectorDetailsModal
                        ...
                        onSaveIndividual={(connectors: Connector[]) => {
                            ...
                            console.log('💾 [Builds Page] Saving individual connector:', connectors);
                            ...
                            if (latestRow && connectors.length > 0) {
                                ...
                                console.log('💾 [Builds Page] Updated row with connectors:', updatedRow.id, 'connectors count:', updatedRow.connectors?.length || 0);
                                console.log('💾 [Builds Page] Connector details:', JSON.stringify(updatedRow.connectors, null, 2));
                                // ============================================================================
                                // TEMPORARY: Save to localStorage (REVERT WHEN APIs ARE READY)
                                // ============================================================================
                                setBuilds((prev: BuildRow[]) => {
                                    const updated = prev.map((row: BuildRow) => 
                                        row.id === latestRow.id ? updatedRow : row
                                    );
                                    // Immediately save to localStorage to ensure persistence
                                    if (USE_LOCAL_STORAGE && selectedAccountId && selectedEnterpriseId) {
                                        setTimeout(() => {
                                            saveBuildsToLocalStorage(updated, selectedAccountId, selectedEnterpriseId);
                                            console.log('💾 [Builds Page] Immediately saved to localStorage after connector update');
                                            // Verify what was saved
                                            const saved = loadBuildsFromLocalStorage(selectedAccountId, selectedEnterpriseId);
                                            const savedRow = saved.find((r: BuildRow) => r.id === latestRow.id);
                                            console.log('💾 [Builds Page] Verification - connectors in saved row:', savedRow?.connectors?.length || 0);
                                        }, 50);
                                    }
                                    return updated;
                                });
            ...
            {/* Integration Artifacts Modal */}
            {selectedRowForIntegrationArtifacts && (
                <IntegrationArtifactsModal
                    isOpen={isIntegrationArtifactsModalOpen}
                    onClose={() => {
                        setIsIntegrationArtifactsModalOpen(false);
                        setSelectedRowForIntegrationArtifacts(null);
                    }}
                    row={selectedRowForIntegrationArtifacts!}
                    selectedAccountId={typeof window !== 'undefined' ? window.localStorage.getItem('selectedAccountId') || '' : ''}
                    selectedAccountName={typeof window !== 'undefined' ? window.localStorage.getItem('selectedAccountName') || '' : ''}
                    selectedEnterpriseId={typeof window !== 'undefined' ? window.localStorage.getItem('selectedEnterpriseId') || '' : ''}
                    selectedEnterprise={selectedEnterprise}
            ...
            {/* BuildsPanel */}
            <BuildsPanel
                isOpen={isBuildsPanelOpen}
                onClose={() => setIsBuildsPanelOpen(false)}
                sidebarWidth={isAIPanelCollapsed ? 0 : 256}
            />

            {/* BuildSlidingPanels */}
            <BuildSlidingPanels
                isOpen={isBuildSlidingPanelsOpen}
                onClose={() => {
                    setIsBuildSlidingPanelsOpen(false);
                    setSelectedBuildRowForSlidingPanels(null);
                }}
                buildData={selectedBuildRowForSlidingPanels || undefined}
            ...
            {/* BuildDetailModal */}
            <BuildDetailModal
                isOpen={isBuildDetailModalOpen}
                onClose={() => {
                    setIsBuildDetailModalOpen(false);
                    setSelectedBuildRowForModal(null);
                }}
                buildRow={selectedBuildRowForModal}
                selectedEnterprise={selectedEnterprise}
                selectedEnterpriseId={selectedEnterpriseId}
                selectedAccountId={selectedAccountId}
                selectedAccountName={selectedAccountName}
