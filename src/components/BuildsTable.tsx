...
const getBuildColor = (buildName: string) => {
    const key = buildName.toLowerCase();
    let hash = 0;
...
export interface BuildRow {
    ...
    artifact: string;
    build: string;
    // Add stages array for expandable sub-rows
    stages?: BuildStage[];
    isTemporary?: boolean;
}

// BuildStage interface for sub-rows (deployment stages)
...
function PhoneMultiSelect({
    ...
    isError?: boolean;
    onDropdownOptionUpdate?: (
        type: 'buildNames' | 'entities' | 'pipelines' | 'statuses',
    ...
    const addNew = async () => {
        ...
        try {
            ...
            if (created) {
                ...
                // Note: PhoneMultiSelect not used in builds - keeping for reference
                // if (onNewItemCreated) {
...
function AsyncChipSelect({
    ...
    compact?: boolean;
    onDropdownOptionUpdate?: (
        type: 'buildNames' | 'descriptions' | 'entities' | 'pipelines',
        action: 'update' | 'delete',
        oldName: string,
        newName?: string,
    ) => Promise<void>;
    ...
    const isOptionInUse = React.useCallback(
        (optionName: string): boolean => {
            ...
            return builds.some((build) => {
                ...
                if (currentRowId && build.id === currentRowId) {
                    return false;
                }

                if (type === 'buildName') {
                    // Never filter build names - show all options
                    return false;
                } else if (type === 'pipeline') {
                    // For pipelines, show all options (no filtering needed)
                    return false;
                }
                return false;
            });
        },
        [builds, type, currentRowId, currentRowEnterprise],
    );
        ...
        try {
            ...
            if (type === 'buildName' && dropdownOptions?.buildNames) {
                ...
                if (
                    ...
                    dropdownOptions.entities.length > 0
                ) {
                ...
                console.log('Calling API: /api/descriptions');
                allData =
                ...
                console.log('Calling API: /api/buildNames');
                allData =
                    (await api.get<Array<{id: string; name: string}>>(
                        '/api/buildNames',
                    )) || [];
    ...
    const addNew = async () => {
        ...
        try {
            ...
            if (type === 'description') {
                ...
                // Entity has predefined options, don't create new ones
                console.log(
            ...
            if (created) {
                ...
                if (onNewItemCreated) {
                    ...
                    switch (type) {
                        case 'buildName':
                            dropdownType = 'buildNames';
                            break;
                        case 'description':
                            dropdownType = 'descriptions';
                            break;
                        case 'pipeline':
                            dropdownType = 'pipelines';
                            break;
                        default:
...
interface BuildsTableProps {
    ...
    compressingStageId?: string | null;
    ...
    onOpenTechnicalUserModal?: (row: BuildRow) => void; // Callback to open technical user modal
}

// Build Stage Sub Row Component
function StageSubRow({
    ...
    stage: BuildStage;
    rowId: string;
    onUpdate: (stageId: string, field: keyof BuildStage, value: string) => void;
        ...
        >
            ...
            >
                ...
                <div className='flex flex-col'>
                    ...
                    <InlineEditableText
                        value={stage.name}
                        onCommit={(value) => onUpdate(stage.id, 'name', value)}
                        placeholder='Enter stage name'
                        className='text-sm px-2 py-1 border border-blue-200 rounded'
                    />
                </div>
                ...
                <div className='flex flex-col'>
                    ...
                    <InlineEditableText
                        value={stage.status}
                        onCommit={(value) =>
                            onUpdate(stage.id, 'status', value)
                        }
                ...
                <div className='flex flex-col'>
                    ...
                    <InlineEditableText
                        value={stage.deployedAt}
                        onCommit={(value) =>
                            onUpdate(stage.id, 'deployedAt', value)
                        }
...
function SortableBuildRow({
    ...
    const editableCols = cols.filter((col) =>
        ['buildName', 'description', 'entity'].includes(col),
    );
            ...
            {cols.includes('artifact') && (
                ...
                >
                    <InlineEditableText
                        value={row.artifact || ''}
                        onCommit={(v) =>
                            onUpdateField(row.id, 'artifact' as any, v)
                        }
                        className='text-[12px]'
                        dataAttr={`artifact-${row.id}`}
                        isError={isCellMissing(row.id, 'artifact')}
                        placeholder='Enter artifact'
                        {...createTabNavigation('artifact')}
                    />
                </div>
            )}
            {cols.includes('build') && (
...
const BuildsTable = forwardRef<any, BuildsTableProps>(
    (
        ...
        // State for expanded rows and stages
        const [expandedRows, setExpandedRows] = useState<Set<string>>(
            new Set(),
        );
        const [rowStages, setRowStages] = useState<
            Record<string, BuildStage[]>
        >({});
        ...
        const [localEdits, setLocalEdits] = useState<
            Record<string, Partial<BuildRow>>
        >({});
        ...
        // Initialize rowStages from rows prop only once
        const [hasInitializedStages, setHasInitializedStages] = useState(false);

        // Expose methods to parent component via ref
        useImperativeHandle(
            ref,
            () => ({
                completeStageDeletion: () => {
                    if (pendingDeleteStageId && pendingDeleteRowId) {
                        console.log(
                            '🗑️ Completing stage deletion via ref:',
                        ...
                        setRowStages((prev) => ({
                            ...
                            [pendingDeleteRowId]: (
                                ...
                                (stage) => stage.id !== pendingDeleteStageId,
                            ),
                        }));
                        setPendingDeleteStageId(null);
                        setPendingDeleteRowId(null);
                        console.log(
                            '✅ Stage removed from rowStages state via ref',
                        );
                    }
                },
            ...
            [pendingDeleteStageId, pendingDeleteRowId, rowStages],
        );
        ...
        const isFieldMissing = (row: BuildRow, field: string): boolean => {
            switch (field) {
                case 'buildName':
                    return !row.buildName || row.buildName.trim() === '';
                case 'description':
        ...
        useEffect(() => {
            if (!hasInitializedStages) {
                const initialStages: Record<string, BuildStage[]> = {};
                rows.forEach((row) => {
                    if (row.stages && row.stages.length > 0) {
                        initialStages[row.id] = row.stages;
                    }
                });

                if (Object.keys(initialStages).length > 0) {
                    setRowStages(initialStages);
                }
                setHasInitializedStages(true);
            }
        }, [rows, hasInitializedStages]);
        ...
        // Only show stage validation during explicit save attempts with incomplete stages
        const prevShowValidationErrors = useRef(false);
        useEffect(() => {
            // Only trigger stage validation when showValidationErrors changes from false to true (save attempt)
            // Stage validation removed - not needed for build stages
        }, [showValidationErrors]);
        async function persistBuildRow(row: BuildRow) {
            try {
                // Skip auto-save for temporary rows - let the parent handle build linkage auto-save
                ...
                const core = {
                    // Core fields for build configuration
                    accountName: row.buildName,
                    masterAccount: row.description,
                    cloudType: row.entity,
                    address: row.pipeline,
                } as any;
                ...
                // Check if we're on build management page
                if (
                    typeof window !== 'undefined' &&
                    window.location.pathname.includes('/manage-accounts')
                ) {
                    console.log(
                        '🔄 Updating build linkage instead of enterprise:',
                        row.id,
                    );

                    // For build management, update the linkage via the parent's onUpdateField
                    // The parent component will handle the build linkage updates
                    console.log(
                        '⏭️ Skipping direct API call for build management page',
                    );
                    return;
                }
                ...
                console.log(
                    '⏭️ Skipping API call - build management handled by parent',
                );
                return;
            } catch (_e) {
        ...
        const addNewStage = (rowId: string) => {
            const newStageId = `stage-${rowId}-${Date.now()}`;
            ...
            // Ensure the row is expanded when adding a stage
            setExpandedRows((prev) => {
                const newSet = new Set(prev);
                newSet.add(rowId);
                return newSet;
            ...
            setRowStages((prev) => ({
                ...prev,
                [rowId]: [...(prev[rowId] || []), newStage],
            }));

            // Update parent state with the new stage
            if (onUpdateField) {
                const currentStages = rowStages[rowId] || [];
                const updatedStages = [...currentStages, newStage];
                onUpdateField(rowId, 'stages', updatedStages);
            }

            // Don't clear validation errors when adding new stage - let existing validation state persist
            // This allows multiple accounts to maintain their validation highlighting

            console.log(
                '➕ Added new empty stage, updated parent state but preserved validation state:',
                {
                    rowId,
                    stageId: newStageId,
                },
        ...
        const updateStage = (
            rowId: string,
            stageId: string,
            field: keyof BuildStage,
            value: string,
        ) => {
            setRowStages((prev) => {
                const updatedStages = {
                    ...prev,
                    [rowId]: (prev[rowId] || []).map((stage) =>
                        stage.id === stageId
                            ? {...stage, [field]: value}
                            : stage,
                    ),
                ...
                // Check if the stage is now complete using the updated state
                return updatedStages;
            });

            // Trigger auto-save for stage updates
            if (onUpdateField) {
                const updatedRowStages =
                    rowStages[rowId]?.map((stage) =>
                        stage.id === stageId
                            ? {...stage, [field]: value}
                            : stage,
                    ) || [];

                onUpdateField(rowId, 'stages', updatedRowStages);
            }
        };

        const deleteStage = async (rowId: string, stageId: string) => {
            ...
            // Trigger auto-save for stage deletion
            if (onUpdateField) {
                const updatedStages = (rowStages[rowId] || []).filter(
                    (stage) => stage.id !== stageId,
                );
                onUpdateField(rowId, 'stages', updatedStages);
            }
        ...
        // Complete the stage deletion when animation and confirmation are done
        const completeStageDeletion = () => {
            if (pendingDeleteStageId && pendingDeleteRowId) {
                ...
                setRowStages((prev) => {
                    ...
                    // Trigger auto-save for stage deletion after animation
                    if (onUpdateField) {
                        onUpdateField(
                            rowId,
                            'stages',
        ...
        const isStageFieldMissing = (
            stage: BuildStage,
            field: keyof BuildStage,
        ): boolean => {
            // Simple check if field is empty
            return !stage[field] || stage[field].toString().trim() === '';
        };
        ...
        const colSizes: Record<string, string> = {
            ...
            buildName: '200px', // Build name column
            description: '250px', // Description column
            entity: '160px', // Entity column
            pipeline: '160px', // Pipeline column
            status: '140px', // Status column
            artifact: '180px', // Artifact column
            build: '140px', // Build column
        };
        ...
        const gridTemplate = useMemo(() => {
            ...
            const base = cols.map((c, index) => {
                ...
                const constraints = {
                    buildName: {min: 180, max: 300},
                    description: {min: 200, max: 400},
                    entity: {min: 140, max: 250},
                    pipeline: {min: 140, max: 250},
                    status: {min: 120, max: 200},
                    artifact: {min: 150, max: 280},
                    build: {min: 120, max: 200},
                };
        ...
        const [internalSortCol, setInternalSortCol] = useState<
            'buildName' | 'email' | 'phone' | 'status' | 'servicesCount' | null
        >(null);
        ...
        const toggleSort = (
            col: 'buildName' | 'email' | 'phone' | 'status' | 'servicesCount',
            direction?: 'asc' | 'desc',
        ) => {
        ...
        const groupedItems = useMemo(() => {
            ...
            displayItems.forEach((item) => {
                ...
                switch (groupBy) {
                    case 'buildName':
                        groupKey = item.buildName || '(No Build Name)';
                        break;
                    case 'description':
                        groupKey = item.description || '(No Description)';
                        break;
                    case 'entity':
        ...
        useEffect(() => {
            const checkScrollNeed = () => {
                ...
                // Increased buffer to compensate for hover scale effects (scale: 1.02 = 2% increase)
                                            ...
                                            {expandedRows.has(r.id) && (
                                                <div className='relative bg-gradient-to-r from-blue-50/80 to-transparent border-l-4 border-blue-400 ml-2 mt-1 mb-2'>
                                                    ...
                                                    <div className='p-3 pb-2'>
                                                        ...
                                                        {/* Render existing deployment stages */}
                                                        <div className='space-y-2'>
                                                            {(
                                                                rowStages[
                                                                    r.id
