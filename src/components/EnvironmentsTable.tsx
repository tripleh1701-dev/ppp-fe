...
// AsyncChipSelect for Environment Name with dropdown and + sign for new values
...
interface EnvironmentsTableProps {
    ...
    onOpenAddressModal?: (row: EnvironmentRow) => void; // Callback to open address modal
    onOpenUserGroupModal?: (row: EnvironmentRow) => void; // Callback to open user group modal
    onOpenScopeModal?: (row: EnvironmentRow) => void; // Callback to open scope config modal
...
function SortableEnvironmentRow({
    ...
    onUpdateField: (rowId: string, key: keyof EnvironmentRow, value: any) => void;
    isSelected: boolean;
    onSelect: (id: string) => void;
    onStartFill: (rowId: string, col: keyof EnvironmentRow, value: string) => void;
    inFillRange: boolean;
    pinFirst?: boolean;
    ...
    onOpenAddressModal?: (row: EnvironmentRow) => void;
    onOpenUserGroupModal?: (row: EnvironmentRow) => void;
    onOpenScopeModal?: (row: EnvironmentRow) => void;
...
const EnvironmentsTable = forwardRef<any, EnvironmentsTableProps>(({
    rows,
    onEdit,
    onDelete,
    title,
    ...
    const [selectedRoleForScope, setSelectedRoleForScope] = useState<EnvironmentRow | null>(null);
    ...
    const prevRowsRef = useRef<EnvironmentRow[]>([]);
    const orderRef = useRef<string[]>([]);
    
    // Keep local state for editing, but initialize it safely
    const [localEdits, setLocalEdits] = useState<Record<string, Partial<EnvironmentRow>>>({});
    ...
    const isFieldMissing = (row: EnvironmentRow, field: string): boolean => {
        switch (field) {
            case 'connectorName':
    ...
    const orderedItems = useMemo(
        () =>
            order
                ...
                .filter(Boolean) as EnvironmentRow[],
        [order, localRows],
    );
    ...
    const latestRowRef = useRef<Record<string, EnvironmentRow>>({});
    function schedulePersist(row: EnvironmentRow, delay = 600) {
        ...
        saveTimersRef.current[rowId] = setTimeout(() => {
            ...
            if (latest) void persistEnvironmentRow(latest);
        }, delay);
    }
    useEffect(() => {
        return () => {
    ...
    async function persistEnvironmentRow(row: EnvironmentRow) {
        try {
    ...
    const checkForDuplicate = (rowId: string, updatedRow: EnvironmentRow): boolean => {
    ...
    function updateRowField(rowId: string, key: keyof EnvironmentRow, value: any) {
        let changed: EnvironmentRow | null = null;
        ...
        setLocalEdits(prev => {
            ...
            if (baseRow) {
                ...
                const next = {...currentRow, [key]: value} as EnvironmentRow;
                ...
                if (['connectorName', 'entity', 'product', 'service'].includes(key as string)) {
                    ...
                    if (isDuplicate) {
                        ...
                        const message = `This combination of Environment Name (${next.connectorName}), Entity (${next.entity}), Product (${next.product}), and Service (${next.service}) already exists in another row. Please use a different combination.`;
    ...
    const isMainRowComplete = (row: EnvironmentRow): boolean => {
        return !!(row.connectorName && row.connectorName.trim() && 
    ...
    const columnOrder: EnvironmentsTableProps['visibleColumns'] = useMemo(
        () => [
            // User group columns
            'connectorName',
            'description',
    ...
    const colSizes = useMemo(() => ({
        ...
        connectorName: '220px', // Environment Name column
    ...
    const gridTemplate = useMemo(() => {
        ...
        const base = cols.map((c, index) => {
            ...
            const constraints = {
                ...
                connectorName: { min: 200, max: 350 }, // Environment Name - increased size
    ...
    const groupedItems = useMemo(() => {
        ...
        displayItems.forEach((item) => {
            ...
            switch (groupBy) {
                case 'connectorName':
                    groupKey = item.connectorName || '(No Environment Name)';
                    break;
                case 'entity':
                            ...
                            {displayItems.map((r, idx) => (
                                <div key={r.id}>
                                    <SortableEnvironmentRow
                                        ...
                                        onOpenScopeModal={onOpenScopeModal || ((row: EnvironmentRow) => {
...
EnvironmentsTable.displayName = 'EnvironmentsTable';
