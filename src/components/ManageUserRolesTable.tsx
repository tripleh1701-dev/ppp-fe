...
function AsyncChipSelectRoleName({
    ...
    const loadAllOptions = useCallback(async () => {
        ...
        try {
            ...
            const availableData = distinctData.filter(
                (item) => !usedRoleNames.has(item.name.toLowerCase().trim()),
            );

            console.log(
                `📋 [RoleName] Total roles: ${transformedData.length}, Distinct role names: ${distinctData.length}`,
            );
            console.log(
                `📋 [RoleName] Already used in table: ${usedRoleNames.size}`,
            );
            console.log(
    ...
    const addNew = async () => {
        ...
        if (isDuplicateInTable) {
            console.log(
                '❌ [RoleName] Duplicate role name detected in table:',
                name,
            );
            alert(
    ...
    const loadAllOptions = useCallback(async () => {
        ...
        try {
            ...
            const filteredByAccountAndEnterprise = response.filter((item) => {
                ...
                const hasEntityName =
                    item.entityName && item.entityName.trim() !== '';

                if (!matchesAccount || !matchesEnterprise) {
                    console.log(
                        '🔍 [Entity] Filtering out item - does not match account/enterprise:',
                        {
            ...
            const uniqueEntities = Array.from(
                new Set(
                    filteredByAccountAndEnterprise.map(
            ...
            console.log(
                '✅ [Entity] Filtered unique entities for account/enterprise:',
                uniqueEntities,
            );
            console.log(
                '🔍 [Entity] Filtered from',
                response.length,
                'total items to',
            ...
            if (uniqueEntities.length === 0) {
                console.log(
                    '🔍 [Entity] DEBUGGING: Expected to find "Finance" entity but got none',
                );
                console.log(
                    '🔍 [Entity] Database has: Accenture Digital + Enterprise Business Suite + Finance',
                );
...
function SortableUserRoleRow({
    ...
    const editableCols = cols.filter((col) =>
        ['roleName', 'description', 'entity', 'product', 'service'].includes(
            col,
        ),
    );
...
const ManageUserRolesTable = forwardRef<any, UserRolesTableProps>(
    (
        ...
        const [selectedRoleForScope, setSelectedRoleForScope] =
            useState<UserRoleRow | null>(null);
        ...
        const [localEdits, setLocalEdits] = useState<
            Record<string, Partial<UserRoleRow>>
        >({});
        ...
        const {baseLocalRows, order} = useMemo(() => {
            ...
            const getRowSignature = (r: UserRoleRow) =>
        ...
        const checkForDuplicate = (
            ...
            const duplicateRow = localRows.find(
                (row) =>
                    ...
                    row.roleName?.trim().toLowerCase() ===
                        updatedRow.roleName?.trim().toLowerCase() &&
        ...
        function updateRowField(
            ...
            setLocalEdits((prev) => {
                ...
                if (baseRow) {
                    ...
                    if (
                        ['roleName', 'entity', 'product', 'service'].includes(
                            key as string,
                        )
                    ) {
