...
export default function EnterpriseConfigTable({
    ...
    function SwipeRow({
        ...
        // Swipe-to-delete deprecated; returning early to use drag-to-trash only
        return (
            <div
        ...
        const progress = useTransform(x, [0, 160], [0, 1]);
        const bgOpacity = useTransform(x, [0, 48, 120], [0, 0.5, 0.95]);
        const iconScale = useTransform(x, [0, 96, 160], [0.9, 1.1, 1.25]);
        const labelOpacity = useTransform(x, [0, 96], [0, 1]);
        const accentWidth = useTransform(x, [0, 160], ['0px', '36px']);
        const tilt = useTransform(x, [0, 160], [0, 0.4]);
        const dropShadow = useTransform(
            x,
            [0, 160],
            ['0 0 0 rgba(0,0,0,0)', '0 8px 24px rgba(244,63,94,0.25)'],
        );
        const [hoveringHandle, setHoveringHandle] = React.useState(false);
        return (
            <motion.div
                layout
                ...
                style={{
                    ...
                    touchAction: 'pan-y',
                    rotateZ: tilt as any,
                    boxShadow: dropShadow as any,
                }}
                onDragEnd={(_, info) => {
                    try {
                        if (info.offset.x > 96) {
                            onSwiped();
                            // Reset position after triggering confirm
                            animate(x, 0, {duration: 0.2});
                        }
                    } catch {}
                }}
                className={`relative select-none w-full grid items-center gap-0 rounded-md overflow-visible border-0 transition-all duration-200 ${
                    idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/70'
                }`}
                draggable={draggable}
            ...
            >
                <motion.div
                    className='absolute inset-0 flex items-center pl-3 pr-2 pointer-events-none'
                    style={{
                ...
                >
                    <motion.div
                        className='flex items-center gap-2 text-rose-700 drop-shadow-[0_1px_2px_rgba(0,0,0,0.25)]'
                ...
                <motion.div
                    ...
                    style={{
                        width: accentWidth,
                        background:
                            'linear-gradient(90deg, rgba(244,63,94,0.9) 0%, rgba(244,63,94,0.45) 60%, rgba(244,63,94,0) 100%)',
                    }}
                />
                {/* Extra accent expansion on hover */}
                <motion.div
                    className='absolute inset-y-0 left-0 pointer-events-none'
                    animate={{width: hoveringHandle ? 56 : 0}}
                    transition={{duration: 0.25}}
                    style={{
                        background:
                            'linear-gradient(90deg, rgba(244,63,94,0.35) 0%, rgba(244,63,94,0.15) 70%, rgba(244,63,94,0) 100%)',
                    }}
                />
                {/* Animated chevrons and hint */}
                <motion.div
                    className='absolute inset-y-0 left-2 flex items-center gap-1 text-rose-600 pointer-events-none'
                    style={{opacity: progress}}
                >
                    <motion.span
                        animate={
                            hoveringHandle
                                ? {x: [0, 6, 0], opacity: [0.5, 1, 0.5]}
                                : {x: 0}
                        }
                        transition={
                            hoveringHandle
                ...
                <div
                    className='absolute inset-y-0 left-0 w-16 z-[999] cursor-ew-resize bg-gradient-to-r from-rose-200/70 via-rose-100/10 to-transparent hover:from-rose-300/80'
                    onPointerDown={(e) => controls.start(e)}
                    onMouseEnter={() => {
                        setHoveringHandle(true);
                        // stronger playful nudge: right then back with overshoot
                        animate(x, [0, 20, 0, 12, 0], {
                            duration: 0.9,
                            ease: 'easeInOut',
                        });
                    }}
