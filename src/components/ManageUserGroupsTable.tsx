        ...
        try {
            ...
            if (created) {
                ...
                if (onNewItemCreated && created) {
                    ...
                    const dropdownType =
                        typeMap[type as string] || 'groupNames';
...
function AsyncChipSelectGroupName({
    ...
    const loadAllOptions = useCallback(async () => {
        ...
        try {
            ...
            const allData =
                (await api.get<Array<{id: string; name: string}>>(groupsUrl)) ||
                [];
            console.log(
                `✅ [GroupName] API call successful, got ${allData.length} items:`,
                allData,
            );
            ...
            const availableData = distinctData.filter(
                (item) => !usedGroupNames.has(item.name.toLowerCase().trim()),
            );

            console.log(
                `📋 [GroupName] Total groups: ${transformedData.length}, Distinct group names: ${distinctData.length}`,
            );
            console.log(
                `📋 [GroupName] Already used in table: ${usedGroupNames.size}`,
            );
            console.log(
                `📋 [GroupName] Available (unused) group names: ${availableData.length}`,
            );
            console.log(
                `📋 [GroupName] Available group names for dropdown:`,
            ...
            setLoading(false);
            console.log(
                '🏁 [GroupName] loadAllOptions completed, loading set to false',
            );
        }
    }, [
        selectedAccountId,
    ...
    const filterOptions = useCallback(() => {
        console.log('🔍 [GroupName] filterOptions called', {
            allOptionsLength: allOptions.length,
            query,
        });
        if (allOptions.length === 0) {
            console.log(
                '⚠️ [GroupName] allOptions is empty, setting options to []',
            );
            setOptions([]);
            return;
        }
        ...
        console.log(
            `🔍 [GroupName] Starting with ${filtered.length} options from API`,
        );
        ...
        if (query) {
            ...
            console.log(
                `🔍 [GroupName] After startsWith filter (${queryLower}): ${filtered.length} items`,
                filtered,
            );
        ...
        console.log(
            `✅ [GroupName] Setting options to ${filtered.length} filtered items`,
        );
        setOptions(filtered);
    }, [allOptions, query]);
    ...
    const addNew = async () => {
        ...
        if (isDuplicateInTable) {
            console.log(
                '❌ [GroupName] Duplicate group name detected in table:',
                name,
            );
            alert(
                `Group name "${name}" already exists in the table. Please use a different name.`,
            );
            return;
        }
        ...
        try {
            ...
            console.log(
                '🌐 [GroupName] Creating group with context:',
                groupData,
            );
                    ...
                    try {
                        ...
                        if (inputRef.current) {
                            ...
                            if (
                                ...
                                inputRef.current.getAttribute('tabindex') !==
                                    null
                            ) {
                                inputRef.current.focus();
                                console.log(
                                    '🎯 [GroupName] Focused chip after creation',
                                );
                            } else {
                                ...
                                if (chipElement) {
                                    chipElement.focus();
                                    console.log(
                                        '🎯 [GroupName] Focused chip after creation (found via querySelector)',
                                    );
                                }
                            }
                        }
                        ...
                        console.log(
                            '🎯 [GroupName] Error focusing chip after creation:',
                            e,
                        );
                    }
            ...
            >
                ...
                {(!current && !value) || open ? (
                    <input
                        ...
                        onChange={(e: any) => {
                            const newValue = e.target.value;
                            console.log('⌨️ [GroupName] onChange:', {
                                newValue,
                                allOptionsLength: allOptions.length,
                                open,
                            });
                            ...
                            if (containerRef.current) {
                                ...
                                const left = containerRect.left;
                                console.log(
                                    '📍 [GroupName] Setting dropdown position:',
                                    {top, left, width},
                                );
                                setDropdownPortalPos({top, left, width});
                            }
                            ...
                            console.log(
                                '📥 [GroupName] Reloading options to filter out used group names',
                            );
                            loadAllOptions();
                            // Clear current selection if user clears the input completely
                            if (newValue === '') {
                        ...
                        onFocus={() => {
                            console.log('👁️ [GroupName] onFocus:', {
                                allOptionsLength: allOptions.length,
                                open,
                                query,
                            });
                            ...
                            if (containerRef.current) {
                                ...
                                const left = containerRect.left;
                                console.log(
                                    '📍 [GroupName] Setting dropdown position on focus:',
                                    {top, left, width},
                                );
                                setDropdownPortalPos({top, left, width});
                            }
                            ...
                            console.log(
                                '📥 [GroupName] Reloading options on focus to filter out used group names',
                            );
                            loadAllOptions();
                        }}
                        onKeyDown={async (e: any) => {
                            if (e.key === 'Enter' && query.trim()) {
                                ...
                                if (exactMatch) {
                                    ...
                                    if (isDuplicate) {
                                        console.log(
                                            '❌ [GroupName] Cannot select duplicate group name:',
                                            exactMatch.name,
                                        );
                                        alert(
                                        ...
                                        try {
                                            ...
                                            if (inputRef.current) {
                                                ...
                                                if (
                                                    ...
                                                    inputRef.current.focus();
                                                    console.log(
                                                        '🎯 [GroupName] Focused chip after Enter on existing value',
                                                    );
                                                } else {
                                                    // If inputRef is still the input, find the chip
                                                    const chipElement =
                                                    ...
                                                    if (chipElement) {
                                                        chipElement.focus();
                                                        console.log(
                                                            '🎯 [GroupName] Focused chip after Enter on existing value (found via querySelector)',
                                                        );
                                                    }
                                                }
                                            }
                                            ...
                                            console.log(
                                                '🎯 [GroupName] Error focusing chip after Enter on existing value:',
                                                e,
                                            );
                                        }
                                    }, 100); // Small delay to ensure React state updates are complete
                                        ...
                                        setTimeout(() => {
                                            try {
                                                ...
                                                if (chipElement) {
                                                    chipElement.focus();
                                                    console.log(
                                                        '🎯 [GroupName] Focused chip after Tab on existing value',
                                                    );
                                                ...
                                                console.log(
                                                    '🎯 [GroupName] Error focusing chip after Tab on existing value:',
                                                    e,
                                                );
                                            }
                                        }, 100);
                    ...
                    >
                        ...
                        <div className='relative z-10 flex flex-col'>
                            ...
                            >
                                {(() => {
                                    console.log(
                                        '🎨 [GroupName] Rendering dropdown content',
                                        {
                                            query: query.trim(),
                                    ...
                                    console.log(
                                        '➕ [GroupName] showCreateNew calculation:',
                                        {
                                    ...
                                    if (
                                        ...
                                        console.log(
                                            '⏳ [GroupName] Showing loading message',
                                        );
                                        return (
                                            <div className='px-3 py-2 text-slate-500 text-center'>
                                                Loading…
                                    ...
                                    if (
                                        ...
                                        console.log(
                                            '🚫 [GroupName] Showing "No matches" message',
                                        );
                                        return (
                                            <div className='px-3 py-2 text-slate-500 text-center'>
                                                No matches
                                    ...
                                    if (
                                        ...
                                        console.log(
                                            '📭 [GroupName] Showing "No value found" message',
                                        );
                                        return (
                                            <div className='px-3 py-2 text-slate-500 text-center'>
                                                No value found
                                    ...
                                    console.log(
                                        '✅ [GroupName] Rendering dropdown items and + button',
                                        {
                                                                    ...
                                                                    () => {
                                                                        try {
                                                                            ...
                                                                            if (
                                                                                ...
                                                                                chipElement.focus();
                                                                                console.log(
                                                                                    '🎯 [GroupName] Focused chip after dropdown selection',
                                                                                );
                                                                            }
                                                                        } catch (e) {
                                                                            console.log(
                                                                                '🎯 [GroupName] Error focusing chip after dropdown selection:',
                                                                                e,
                                                                            );
                                                                        }
                                                                    },
                                                ...
                                                >
                                                    <button
                                                        onClick={(e) => {
                                                            ...
                                                            e.stopPropagation();
                                                            console.log(
                                                                '🖱️ [GroupName] + button clicked for:',
                                                                query.trim(),
                                                            );
                                                            addNew();
                                                        }}
...
function SortableUserGroupRow({
    ...
    const editableCols = cols.filter((col) =>
        ['groupName', 'description', 'entity', 'product', 'service'].includes(
            col,
        ),
    );
                    ...
                    >
                        {enableDropdownChips ? (
                            <AsyncChipSelectGroupName
                                ...
                                placeholder=''
                                isError={
                                    isCellMissing(row.id, 'groupName') ||
                                    !!(
...
const ManageUserGroupsTable = forwardRef<any, UserGroupsTableProps>(
    (
        ...
        const [localEdits, setLocalEdits] = useState<
            Record<string, Partial<UserGroupRow>>
        >({});
        ...
        const {baseLocalRows, order} = useMemo(() => {
            ...
            const getRowSignature = (r: UserGroupRow) =>
                `${r.id}|${r.groupName || ''}|${r.description || ''}|${
                    r.entity || ''
                }|${r.product || ''}|${r.service || ''}|${
                    r.assignedRoles?.length || 0
                }`;
            ...
            const duplicateRow = localRows.find(
                (row) =>
                    ...
                    row.groupName?.trim().toLowerCase() ===
                        updatedRow.groupName?.trim().toLowerCase() &&
        ...
        function updateRowField(
            ...
            setLocalEdits((prev) => {
                ...
                if (baseRow) {
                    ...
                    if (
                        ['groupName', 'entity', 'product', 'service'].includes(
                            key as string,
                        )
                    ) {
